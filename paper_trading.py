"""
=============================================================
  CRYPTO AI BOT — Paper Trading v8
=============================================================
  Corre cada hora via GitHub Actions.
  Usa CryptoCompare API (sin restricciones geográficas).
...
def cargar_estado() -> Dict:
    ...
    return {
        ...
        'n_operaciones': 0, 'n_wins': 0, 'n_loses': 0,
        'pnl_total': 0.0, 'funding_total': 0.0,
        'ultima_ejecucion': None, 'version': 'v8',
    }
...
def actualizar_metricas(estado: Dict) -> Dict:
...
def calcular_indicadores(df: pd.DataFrame) -> pd.DataFrame:
    """Calcula todos los indicadores necesarios para el modelo."""
    try:
    ...
    if macd_df is not None and not macd_df.empty:
        ...
        # Buscar columnas por posición (compatibilidad pandas-ta vs pandas-ta-openbb)
        df['macd']        = macd_df.iloc[:, 0]
        df['macd_signal'] = macd_df.iloc[:, 1] if len(cols) > 1 else np.nan
        df['macd_hist']   = macd_df.iloc[:, 2] if len(cols) > 2 else np.nan
    else:
    ...
    if stoch_df is not None and not stoch_df.empty:
        ...
        df['stoch_d'] = stoch_df.iloc[:, 1] if stoch_df.shape[1] > 1 else np.nan
    ...
    if bb_df is not None and not bb_df.empty:
        # Buscar columnas por nombre parcial para compatibilidad
        bb_cols = bb_df.columns.tolist()
        upper_col = next((c for c in bb_cols if 'U' in c or 'upper' in c.lower()), None)
        mid_col   = next((c for c in bb_cols if 'M' in c or 'mid' in c.lower()), None)
        lower_col = next((c for c in bb_cols if 'L' in c or 'lower' in c.lower()), None)
        df['bb_upper'] = bb_df[upper_col] if upper_col else np.nan
        df['bb_mid']   = bb_df[mid_col]   if mid_col   else np.nan
        df['bb_lower'] = bb_df[lower_col] if lower_col else np.nan
        ...
        for c in ['bb_upper', 'bb_mid', 'bb_lower', 'bb_width', 'bb_posicion', 'bb_squeeze']:
            df[c] = np.nan
    ...
    if kc_df is not None and not kc_df.empty:
        ...
        df['kc_mid']   = kc_df.iloc[:, 1] if kc_df.shape[1] > 1 else np.nan
        df['kc_lower'] = kc_df.iloc[:, 2] if kc_df.shape[1] > 2 else np.nan
    else:
    ...
    try:
        ...
        vwap_result = ta.vwap(df_temp['high'], df_temp['low'],
    ...
    ultima_res = high.where(df['es_resistencia'] == 1).ffill()
    ultimo_sop = low.where(df['es_soporte'] == 1).ffill()
    df['dist_resistencia_pct'] = (ultima_res - close) / close * 100
    df['dist_soporte_pct']     = (close - ultimo_sop) / close * 100
    ...
    for nivel in ['fib_236', 'fib_382', 'fib_500', 'fib_618']:
        df[f'cerca_{nivel}'] = (abs(close - df[nivel]) / close < 0.005).astype(int)

    # ── Régimen de mercado ──
    df['ema200_pendiente'] = (df['ema_200'].diff(periods=50)
                              / df['ema_200'].shift(50) * 100)
...
def cargar_modelo(simbolo: str) -> Optional[Dict]:
    ...
    if not os.path.exists(os.path.join(dir_m, 'xgboost.pkl')):
        print(f"  ❌ Modelo no encontrado: {dir_m}")
        return None
    try:
...
def predecir(df: pd.DataFrame, modelos: Dict) -> float:
    ...
    return float(modelos['ensemble'].predecir_proba(probs_xgb, probs_lstm)[-1])
...
def calcular_sl_tp(precio: float, atr: float,
    ...
    sl_pct = float(np.clip((atr * cfg['sl_mult']) / precio, 0.008, cfg['sl_max']))
    tp_pct = float(np.clip(sl_pct * cfg['tp_ratio'], sl_pct * 2.5, cfg['tp_max']))
    liq    = precio * (1 - (1 / cfg['leverage']) * 0.90)
    return precio * (1 - sl_pct), precio * (1 + tp_pct), liq
...
def gestionar_posiciones(estado: Dict, precio_actual: float,
    ...
    for pos in estado['posiciones']:
        ...
        if precio_actual > pos.get('precio_max', pos['precio_entrada']):
            ...
            ganancia = (precio_actual - pos['precio_entrada']) / pos['precio_entrada']
            if ganancia >= TRAILING_ACTIVACION:
                pos['trailing'] = True
        ...
        if cerrar:
            ...
            op = {
                'timestamp': timestamp, 'simbolo': simbolo,
                'tipo': 'cierre', 'motivo': motivo,
...
def abrir_posicion(estado: Dict, simbolo: str, precio: float,
    ...
    margen_en_uso  = sum(p['margen'] for p in estado['posiciones']
    ...
    enviar_telegram(
        ...
        f"Precio: ${p_ent:,.4f}\n"
        f"SL: ${sl:,.4f} | TP: ${tp:,.4f}\n"
        f"Margen: ${margen_real:.2f} | x{cfg['leverage']}\n"
        f"Capital: ${estado['capital']:,.2f}"
    )
...
def ejecutar():
    ...
    hora_actual = int(time.time() / 3600)
    ...
    if estado['en_pausa']:
        ...
        if hora_actual < fin_pausa:
            print(f"  ⏸️  Bot en pausa. Quedan {fin_pausa - hora_actual}h")
            guardar_estado(estado)
            return
        else:
            estado['en_pausa'] = False
            print("  ▶️  Pausa terminada.")
    ...
    for simbolo in CRIPTOS:
        ...
        if len(df) < 50:
            print(f"  ⚠️  Pocas velas válidas: {len(df)}")
            continue

        # Última vela completa (penúltima, la última está en formación)
        ultima        = df.iloc[-2]
        ...
        if pd.isna(atr_actual) or atr_actual <= 0:
            atr_actual = precio_actual * 0.015
        ...
        # Verificar drawdown
        estado['capital_max'] = max(estado['capital_max'], estado['capital'])
        ...
        if dd >= DD_PAUSA_PCT and not estado['en_pausa']:
            ...
            enviar_telegram(f"⏸️ <b>PAUSA {HORAS_PAUSA}h</b>\nDrawdown: {dd*100:.1f}%")
            continue

        # Verificar límite de posiciones
        n_pos = sum(1 for p in estado['posiciones'] if p['simbolo'] == nombre)
        if n_pos >= MAX_POSICIONES:
            print(f"  ℹ️  Máximo de posiciones para {nombre}")
            continue
        ...
        try:
            prob = predecir(df.iloc[:-1], modelos)
        except Exception as e:
            print(f"  ❌ Error predicción {simbolo}: {e}")
            continue

        print(f"  Prob: {prob:.4f} | Régimen: {regimen} | Precio: ${precio_actual:,.4f}")

        if prob < UMBRAL_MINIMO:
            print(f"  → Sin señal (prob {prob:.3f} < {UMBRAL_MINIMO})")
            continue

        ok, motivo = filtro_entrada(regimen, vol_bajo)
        if not ok:
            print(f"  → Filtrado: {motivo}")
            continue
        ...
        abrir_posicion(estado, nombre, precio_actual, atr_actual,
                       prob, cfg, calidad, timestamp, hora_actual)
        registrar_operacion({
            'timestamp': timestamp, 'simbolo': nombre,
            'tipo': 'apertura', 'motivo': calidad,
            'precio_entrada': round(precio_actual, 6),
            'precio_salida': None, 'pnl_usdt': None,
            'funding_usdt': None, 'horas': None,
            'leverage': cfg['leverage'], 'calidad': calidad,
            'capital_total': round(estado['capital'], 2),
        })
    ...
    print(f"\n{'─'*60}")
    s = '+' if metricas['rentabilidad_pct'] >= 0 else ''
    print(f"  Capital:    ${metricas['capital_actual']:,.2f}  "
          f"({s}{metricas['rentabilidad_pct']:.2f}%)")
    print(f"  Ops:        {metricas['n_operaciones']}  |  "
    ...
    if estado['n_operaciones'] > 0 and estado['n_operaciones'] % 10 == 0:
        ...
        enviar_telegram(
            ...
            f"Capital: ${metricas['capital_actual']:,.2f} ({s}{rent:.2f}%)\n"
            f"Ops: {metricas['n_operaciones']} | WR: {metricas['win_rate_pct']:.1f}%\n"
            f"P&L: ${metricas['pnl_total']:+.2f}"
        )
