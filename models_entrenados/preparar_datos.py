...
def preparar_datos(simbolo, dir_features="data/features", dir_modelos="models", longitud_lstm=48):
    ...
    y_train = train[TARGET_PRINCIPAL].values
    y_val   = val[TARGET_PRINCIPAL].values
    y_test  = test[TARGET_PRINCIPAL].values
    print(f"\n  ⚖️  Balance de clases:")
    print(f"    Train — subida: {y_train.mean()*100:.1f}%  no-subida: {(1-y_train.mean())*100:.1f}%")
    print(f"    Val   — subida: {y_val.mean()*100:.1f}%")
