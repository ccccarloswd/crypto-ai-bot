...
LONG_CONFIG = {
    'elite':   {'leverage': 8, 'margen': 0.22, 'sl_mult': 1.2,
                'sl_max': 0.020, 'tp_ratio': 2.8, 'tp_max': 0.060},
    'premium': {'leverage': 5, 'margen': 0.20, 'sl_mult': 1.5,
                'sl_max': 0.025, 'tp_ratio': 2.5, 'tp_max': 0.050},
}
SHORT_CONFIG = {
    'elite':   {'leverage': 6, 'margen': 0.18, 'sl_mult': 1.3,
                'sl_max': 0.022, 'tp_ratio': 2.8, 'tp_max': 0.055},
    'premium': {'leverage': 4, 'margen': 0.15, 'sl_mult': 1.5,
...
def margen_efectivo(capital: float, cfg_margen: float, af: float) -> float:
    cap = min(capital, CAPITAL_INICIAL * CAP_MULT_FUNDING)
    return cap * cfg_margen * af
...
def ejecutar():
    ...
    print(f"  Estrategia: PREDICTIVA con contexto 4H")
    print(f"{'='*55}")
    ...
    if estado.get('primera_ejecucion', True):
        telegram(f"🤖 <b>BOT {VERSION} INICIADO</b>\n"
                 ...
                 f"Estrategia PREDICTIVA:\n"
                 f"  · Score RSI sobrecomprado/vendido\n"
                 f"  · Divergencias RSI\n"
                 f"  · Contexto 4H integrado\n"
                 f"  · Prob alta → menos filtros\n"
                ...
                if prob_long >= PROB_PREMIUM_LONG and score_l >= min_s_l:
                    ...
                    if res:
                        ...
                        abrir_posicion(estado, simbolo, precio, atr,
                                       prob_long, cfg, cal, 'long', ts, hora_unix, razones_l)
                ...
                if sub_short:
                    ...
                    if prob_short >= PROB_PREMIUM_SHORT and score_s >= min_s_s:
                        ...
                        if res:
                            cal, cfg = res
                            abrir_posicion(estado, simbolo, precio, atr,
                                           prob_short, cfg, cal, 'short', ts, hora_unix, razones_s)
