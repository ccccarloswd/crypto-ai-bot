...
def abrir_posicion(estado: Dict, simbolo: str, precio: float, atr: float,
    ...
    # nivel3/nivel4: exigen que 4H acompañe la dirección
    # nivel2: permite 4H lateral pero bloquea si 4H va en contra
    # nivel1: sin restricción de 4H
    t4h = (ctx4h or {}).get('tendencia', 'lateral')
    if nivel_str in ('nivel3', 'nivel4'):
        if dir_ == 'short' and t4h != 'bajista':
            print(f"  ❌ {nivel_str.upper()} SHORT bloqueado — 4H debe ser BAJISTA (es {t4h.upper()})")
            return
        if dir_ == 'long' and t4h != 'alcista':
            print(f"  ❌ {nivel_str.upper()} LONG bloqueado — 4H debe ser ALCISTA (es {t4h.upper()})")
            return
    elif nivel_str == 'nivel2':
        if dir_ == 'short' and t4h == 'alcista':
            print(f"  ❌ NIVEL2 SHORT bloqueado — 4H ALCISTA en contra")
            return
        if dir_ == 'long' and t4h == 'bajista':
            print(f"  ❌ NIVEL2 LONG bloqueado — 4H BAJISTA en contra")
            return
