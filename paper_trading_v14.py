...
=============================================================
  ...
  Mejoras de agotamiento de tendencia:
  · agotamiento_alcista(): detecta pérdida de impulso en
    una subida → habilita shorts anticipados (contra tendencia)
  · agotamiento_bajista(): detecta pérdida de impulso en
    una caída → habilita longs anticipados (contra tendencia)
  · En modo agotamiento: solo calidad ELITE, SL más ajustado
    y margen reducido para compensar el mayor riesgo
...
# ── Configuración de operaciones en modo AGOTAMIENTO ──────────────
# Cuando el bot entra contra tendencia (porque detecta agotamiento),
# usamos parámetros más conservadores: solo ELITE, SL más corto,
# margen reducido y TP igual de ambicioso para que el RR lo justifique.
AGOT_SHORT_ELITE = {
    'prob': 0.70, 'leverage': 4, 'margen': 0.12,
    'sl_mult': 1.0, 'sl_max': 0.016, 'tp_ratio': 3.0, 'tp_max': 0.050,
}
AGOT_LONG_ELITE = {
    'prob': 0.74, 'leverage': 5, 'margen': 0.14,
    'sl_mult': 1.0, 'sl_max': 0.016, 'tp_ratio': 3.0, 'tp_max': 0.055,
}
...
# ──────────────────────────────────────────────
#  ESTRATEGIA v14 — con detección de agotamiento
# ──────────────────────────────────────────────

def agotamiento_alcista(df: pd.DataFrame) -> bool:
    """
    Detecta si una subida está perdiendo fuerza aunque los indicadores
    todavía no sean bajistas. Señal para buscar shorts anticipados.
    Se necesitan ≥2 de 3 condiciones para activarse.

    Condiciones:
      1. RSI venía de sobrecompra (≥68) y ha bajado ≥2.5 puntos en 2 velas
      2. MACD histograma positivo pero decreciendo 2 velas consecutivas
      3. Volumen de las últimas 2 velas por debajo del 85% de su media de 20
    """
    if len(df) < 5:
        return False
    try:
        ...
        if len(rsi) >= 3:
            ...
            rsi_hace2  = float(rsi.iloc[-3])
            if rsi_hace2 >= 68 and rsi_actual < rsi_hace2 - 2.5:
                senal += 1
        ...
        if len(vr) >= 2:
            if float(vr.iloc[-1]) < 0.85 and float(vr.iloc[-2]) < 0.85:
                senal += 1
...
def agotamiento_bajista(df: pd.DataFrame) -> bool:
    """
    Espejo de agotamiento_alcista. Detecta si una caída está perdiendo
    fuerza aunque los indicadores todavía no sean alcistas.
    Señal para buscar longs anticipados.
    Se necesitan ≥2 de 3 condiciones para activarse.

    Condiciones:
      1. RSI venía de sobreventa (≤32) y ha subido ≥2.5 puntos en 2 velas
      2. MACD histograma negativo pero creciendo 2 velas consecutivas
         (menos negativo = impulso bajista perdiendo fuerza)
      3. Volumen de las últimas 2 velas por debajo del 85% de su media de 20
         (la venta se está secando)
    """
    if len(df) < 5:
        return False
    try:
        ...
        if len(rsi) >= 3:
            rsi_actual = float(rsi.iloc[-1])
            rsi_hace2  = float(rsi.iloc[-3])
            if rsi_hace2 <= 32 and rsi_actual > rsi_hace2 + 2.5:
                senal += 1
        ...
        if len(mh) >= 3:
            ...
            mh_prev = float(mh.iloc[-2])
            mh_ant  = float(mh.iloc[-3])
            # Negativo pero subiendo (acercándose a cero) = pérdida de impulso bajista
            if mh_act < 0 and mh_act > mh_prev > mh_ant:
                senal += 1
...
def clasificar_long(prob: float, row: pd.Series,
                    modo_agotamiento: bool = False) -> Optional[Tuple[str, dict]]:
    """
    Clasificación de longs.
    - Modo normal: requiere régimen >= 0 para elite, sin restricción para premium.
    - Modo agotamiento: permite operar aunque el régimen sea bajista (-1 o -2),
      pero solo señales ELITE con parámetros más conservadores.
    """
    reg = int(row.get('regimen_mercado', 0)) if not pd.isna(row.get('regimen_mercado', 0)) else 0

    if modo_agotamiento:
        # No permitimos long de agotamiento si la caída aún está acelerando
        # (regime -2 con mucho momentum es demasiado arriesgado)
        if prob >= AGOT_LONG_ELITE['prob']:
            print(f"  ⚡ Long AGOTAMIENTO elite (prob={prob:.3f}, reg={reg})")
            return 'elite_agot', AGOT_LONG_ELITE
        return None
    else:
...
def confirmacion_short(row: pd.Series, modo_agotamiento: bool = False) -> bool:
    """
    Confirmación estándar: exige que los indicadores ya sean bajistas.
    En modo agotamiento las condiciones se relajan porque los indicadores
    todavía pueden ser alcistas — el agotamiento ya se verificó antes.
    """
    conds = 0
    rsi   = row.get('rsi_14', 50)

    if modo_agotamiento:
        # RSI puede estar alto, solo bloqueamos si sigue acelerando (>76)
        if not pd.isna(rsi) and float(rsi) < 76:
            conds += 1
        # MACD: aceptamos cualquier valor, ya confirmamos que decrece en agotamiento_alcista
        conds += 1
        # Precio: no bloqueamos por posición respecto a medias en modo agotamiento
        conds += 1
    else:
        if not pd.isna(rsi) and float(rsi) < 45:
            conds += 1
        mh = row.get('macd_hist', 0)
        if not pd.isna(mh) and float(mh) < 0:
            conds += 1
...
def confirmacion_long(row: pd.Series, modo_agotamiento: bool = False) -> bool:
    """
    Espejo de confirmacion_short. Confirma condiciones para un long.
    Modo normal: indicadores claramente alcistas.
    Modo agotamiento: condiciones relajadas porque los indicadores aún
    pueden ser bajistas — el agotamiento ya se verificó antes.
    """
    conds = 0
    rsi   = row.get('rsi_14', 50)

    if modo_agotamiento:
        # RSI puede estar bajo, solo bloqueamos si sigue acelerando a la baja (<24)
        if not pd.isna(rsi) and float(rsi) > 24:
            conds += 1
        # MACD: aceptamos cualquier valor, ya confirmamos que crece en agotamiento_bajista
        conds += 1
        # Precio: no bloqueamos por posición respecto a medias en modo agotamiento
        conds += 1
    else:
        if not pd.isna(rsi) and float(rsi) > 55:
            conds += 1
        mh = row.get('macd_hist', 0)
        if not pd.isna(mh) and float(mh) > 0:
            conds += 1
...
def clasificar_short(prob: float, row: pd.Series,
                     modo_agotamiento: bool = False) -> Optional[Tuple[str, dict]]:
    """
    Clasificación de shorts.
    - Modo normal: requiere régimen <= -1 y confirmación bajista clásica.
    - Modo agotamiento: permite operar en régimen alcista, pero solo señales
      ELITE con parámetros conservadores (menor apalancamiento, SL más corto).
    """
    reg = int(row.get('regimen_mercado', 0)) if not pd.isna(row.get('regimen_mercado', 0)) else 0

    if modo_agotamiento:
        if not confirmacion_short(row, modo_agotamiento=True):
            return None
        if prob >= AGOT_SHORT_ELITE['prob']:
            print(f"  ⚡ Short AGOTAMIENTO elite (prob={prob:.3f}, reg={reg})")
            return 'elite_agot', AGOT_SHORT_ELITE
        return None
    else:
        if reg > SHORT_CONFIG['regimen_max']:
            return None
        if not confirmacion_short(row, modo_agotamiento=False):
            return None
        if prob >= SHORT_CONFIG['elite']['prob']:
            return 'elite', SHORT_CONFIG['elite']
        if prob >= SHORT_CONFIG['premium']['prob']:
    ...
    for simbolo in CRIPTOS:
        ...
        if total<MAX_POSICIONES:
            ...
            if sub_long:
                ...
                # Detectar agotamiento bajista (caída perdiendo fuerza → long anticipado)
                agot_baj = agotamiento_bajista(df.iloc[:-1])
                if agot_baj:
                    print(f"  ⚡ Agotamiento bajista detectado")
                if n_longs==0 and prob_long>=LONG_CONFIG['premium']['prob']:
                    res=clasificar_long(prob_long,ultima, modo_agotamiento=agot_baj)
                    if res:
                        cal,cfg=res
            ...
            if len(estado['posiciones'])<MAX_POSICIONES:
                ...
                if sub_short:
                    ...
                    # Detectar agotamiento alcista (subida perdiendo fuerza → short anticipado)
                    agot_alc = agotamiento_alcista(df.iloc[:-1])
                    if agot_alc:
                        print(f"  ⚡ Agotamiento alcista detectado")
                    ...
                    if n_shorts==0 and prob_short>=SHORT_CONFIG['premium']['prob'] and max_g<0.03:
                        res=clasificar_short(prob_short,ultima, modo_agotamiento=agot_alc)
                        if res:
                            cal,cfg=res
