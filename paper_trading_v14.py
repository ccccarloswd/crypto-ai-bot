...
def ejecutar():
    ...
    for simbolo in CRIPTOS:
        ...
        if total >= MAX_POSICIONES:
            print(f"  ⛔ Límite de posiciones alcanzado ({total}/{MAX_POSICIONES}), skip")
            resumen_ciclo.append(f"{simbolo:<12} ⛔  límite posiciones")
            continue
        ...
        if n_longs > 0:
            ...
            if sub_long:
                ...
                print(f"  IA prob={prob_long:.4f}  score_base={score_l:.1f}  "
                ...
                if ctx_señal_l['bloquear']:
                    ...
                    if prob_long < PROB_PREMIUM_LONG:
                        ...
                        if res:
                            cal, cfg = res
                            cfg_mod = dict(cfg)
                            cfg_mod['margen'] = cfg['margen'] * ctx_señal_l['multiplicador_margen']
                            print(f"  🟢 SEÑAL LONG {cal.upper()} — ABRIENDO POSICIÓN")
                            abrir_posicion(estado, simbolo, precio_entrada, atr,
                                           ...
                                           df_pred, ctx4h)
                            resumen_l = f"✅ LONG {cal.upper()} abierto"
                        else:
                            resumen_l = "no abre — clasificación fallida"
            else:
        ...
        elif len(estado['posiciones']) < MAX_POSICIONES:
            max_g_long = max(
                ...
                default=0.0)
            if max_g_long >= 0.03:
                print(f"  ⏸️  Long con ganancia {max_g_long*100:.1f}% — no se abre short simultáneo")
                resumen_s = f"pausado (long +{max_g_long*100:.1f}%)"
            else:
                ...
                if sub_short:
                    ...
                    print(f"  IA prob={prob_short:.4f}  score_base={score_s:.1f}  "
                    ...
                    if ctx_señal_s['bloquear']:
                        ...
                        if prob_short < PROB_PREMIUM_SHORT:
                            ...
                            if res:
                                cal, cfg = res
                                cfg_mod = dict(cfg)
                                cfg_mod['margen'] = cfg['margen'] * ctx_señal_s['multiplicador_margen']
                                print(f"  🔴 SEÑAL SHORT {cal.upper()} — ABRIENDO POSICIÓN")
                                abrir_posicion(estado, simbolo, precio_entrada, atr,
                                               prob_short, cfg_mod, cal, 'short', ts, hora_unix,
                                               razones_s + ctx_señal_s['contexto_log'],
                                               df_pred, ctx4h)
                                resumen_s = f"✅ SHORT {cal.upper()} abierto"
                            else:
                                resumen_s = "no abre — clasificación fallida"
                else:
                    print("  ⚠️  Sin modelo SHORT disponible")
                    resumen_s = "sin modelo"
        else:
            resumen_s = "límite posiciones"
