    ...
    for simbolo in CRIPTOS:
        ...
        if n_longs > 0:
            ...
            if sub_long:
                ...
                # prob_long actuaba como portero: bloqueaba si < PROB_PREMIUM_LONG
                # prob_ct_long: indicador de score corto plazo (ya estaba inactivo)
                # Se conserva el código para reactivar cuando el modelo esté calibrado.
                #
                # prob_long    = predecir(df_pred, sub_long)
                # prob_ct_long = predecir(df_pred.tail(50).reset_index(drop=True), sub_long)
                #
                prob_long    = 1.0   # IA desactivada: prob neutra — el score técnico decide
                prob_ct_long = 0.5   # IA corto plazo desactivada: neutral
                # ── FIN BLOQUE IA ─────────────────────────────────────────────────
                ...
                # ── PRINTS ORIGINALES COMENTADOS ────────────────────────────────
                # print(f"  IA prob={prob_long:.4f}  score_base={score_l:.1f}  "
                #       f"mult_ctx=×{ctx_señal_l['multiplicador_score']:.2f}  "
                #       f"marg=×{ctx_señal_l['multiplicador_margen']:.2f}  "
                #       f"confianza={ctx_señal_l['nivel_confianza'].upper()}")
                # print(f"  🤖 IA_ct (solo info, inactivo)={prob_ct_long:.4f}")
                # ── FIN PRINTS ORIGINALES ─────────────────────────────────────
                ...
                if ctx_señal_l['bloquear']:
                    ...
                    if prob_long < PROB_PREMIUM_LONG:
                        # ── IA DESACTIVADA: este bloque nunca se ejecuta (prob_long=1.0 fijo) ──
                        # Código original conservado comentado para restaurar si se reactiva la IA:
        ...
        if n_shorts > 0:
            ...
            if sub_short:
                # ── IA DESACTIVADA — predicciones del modelo comentadas ──────────
                # prob_short actuaba como portero: bloqueaba si < PROB_PREMIUM_SHORT
                # prob_ct_short: indicador de score corto plazo (ya estaba inactivo)
                # Se conserva el código para reactivar cuando el modelo esté calibrado.
                #
                # prob_short    = predecir(df_pred, sub_short)
                # prob_ct_short = predecir(df_pred.tail(50).reset_index(drop=True), sub_short)
                #
                prob_short    = 1.0   # IA desactivada: prob neutra — el score técnico decide
                prob_ct_short = 0.5   # IA corto plazo desactivada: neutral
                # ── FIN BLOQUE IA ─────────────────────────────────────────────────
                ...
                #       f"marg=×{ctx_señal_s['multiplicador_margen']:.2f}  "
                #       f"confianza={ctx_señal_s['nivel_confianza'].upper()}")
                # print(f"  🤖 IA_ct (solo info, inactivo)={prob_ct_short:.4f}")
                # ── FIN PRINTS ORIGINALES ─────────────────────────────────────
                ...
                if ctx_señal_s['bloquear']:
                    ...
                    if prob_short < PROB_PREMIUM_SHORT:
                        # ── IA DESACTIVADA: este bloque nunca se ejecuta (prob_short=1.0 fijo) ──
                        # Código original conservado comentado para restaurar si se reactiva la IA:
