...
def score_predictivo_short(df: pd.DataFrame, ultima: pd.Series,
    ...
    umbral_sc, umbral_sv = calcular_rsi_umbral(rsi_serie, 'short')
    if rsi >= umbral_sc:
        score += 1.5; razones.append(f"RSI={rsi:.0f} sobrecomprado ✅ (umbral={umbral_sc:.0f})")
    elif rsi >= umbral_sc - 8:
        score += 0.5
    elif rsi <= umbral_sv:
        score -= 1.5; razones.append(f"⚠️ RSI={rsi:.0f} sobrevendido — zona peligrosa para SHORT")
    elif rsi <= umbral_sv + 8:
        score -= 0.5; razones.append(f"⚠️ RSI={rsi:.0f} cerca de sobreventa")
    ...
    if len(mh) >= 3:
        ...
        # MACD positivo y perdiendo fuerza → buena señal SHORT (momentum agotándose)
        if mh_act > 0 and mh_act < mh_prev < mh_ant:
            score += 1.5; razones.append("MACD hist positivo 2 velas decelerando ✅ (pico doble, momentum agotado)")
        elif mh_act > 0 and mh_act < mh_prev:
            score += 1.0; razones.append("MACD hist positivo decelerando ✅ (pico)")
        # MACD negativo y acelerando → confirma bajada ya en curso, señal débil para SHORT predictivo
        elif mh_act < 0 and mh_act < mh_prev:
            score += 0.5; razones.append("MACD hist negativo acelerando (confirma bajada)")
        # MACD positivo y acelerando → fuerza alcista, EN CONTRA del SHORT
        elif mh_act > 0 and mh_act > mh_prev > mh_ant:
            score -= 1.5; razones.append("⚠️ MACD hist positivo acelerando — fuerza alcista en contra del SHORT")
        elif mh_act > 0 and mh_act > mh_prev:
            score -= 0.8; razones.append("⚠️ MACD hist positivo ganando fuerza")
    # Cruce bajista inminente: MACD a punto de cruzar señal a la baja desde positivo
    if len(macd_line) >= 2 and len(macd_sig) >= 2:
        ...
        dist_actual = ml_act - ms_act; dist_prev = ml_prev - ms_prev
        if dist_actual > 0 and dist_actual < dist_prev * 0.3 and dist_prev > 0:
            score += 1.2; razones.append("MACD cruce bajista inminente ✅ (líneas convergiendo desde positivo)")
    ...
    if ctx4h.get('disponible'):
        ...
        # ── MACD 4H: cruce alcista penaliza el SHORT fuertemente ──────────
        if ctx4h.get('macd_cruce_alcista'):
            score -= 2.5
            razones.append("⚠️ ⚠️ MACD 4H cruce alcista CONFIRMADO — señal alcista fuerte en contra del SHORT")
        elif ctx4h.get('macd_cruce_alcista_inm'):
            score -= 1.5
            razones.append("⚠️ MACD 4H cruce alcista inminente — momentum 4H girando alcista")
        elif ctx4h.get('macd_cruce_bajista'):
            score += 1.5
            razones.append("MACD 4H cruce bajista CONFIRMADO ✅ — señal bajista fuerte")
        elif ctx4h.get('macd_cruce_bajista_inm'):
            score += 0.8
            razones.append("MACD 4H cruce bajista inminente ✅")
        # Penalizar también cuando el histograma 4H lleva velas mejorando desde negativo
        mh4     = ctx4h.get('macd_hist', 0)
        mh4_prev = ctx4h.get('macd_hist_prev', 0)
        if mh4 < 0 and mh4 > mh4_prev:
            score -= 1.0
            razones.append("⚠️ MACD hist 4H negativo pero mejorando — momentum bajista agotándose")
...
def score_predictivo_long(df: pd.DataFrame, ultima: pd.Series,
    ...
    rsi = float(ultima.get('rsi_14', 50))
    rsi_serie = df['rsi_14'] if 'rsi_14' in df.columns else None
    umbral_sc_l, umbral_sv_l = calcular_rsi_umbral(rsi_serie, 'long')
    if rsi <= umbral_sv_l:
        score += 1.5; razones.append(f"RSI={rsi:.0f} sobrevendido ✅ (umbral={umbral_sv_l:.0f})")
    elif rsi <= umbral_sv_l + 8:
        score += 0.5
    elif rsi >= umbral_sc_l:
        score -= 1.5; razones.append(f"⚠️ RSI={rsi:.0f} sobrecomprado — zona peligrosa para LONG")
    elif rsi >= umbral_sc_l - 8:
        score -= 0.5; razones.append(f"⚠️ RSI={rsi:.0f} cerca de sobrecompra")
    ...
    mh = df['macd_hist'].dropna()
    macd_line_l = df['macd'].dropna() if 'macd' in df.columns else pd.Series(dtype=float)
    macd_sig_l  = df['macd_signal'].dropna() if 'macd_signal' in df.columns else pd.Series(dtype=float)
    if len(mh) >= 3:
        mh_act, mh_prev, mh_ant = float(mh.iloc[-1]), float(mh.iloc[-2]), float(mh.iloc[-3])
        # MACD negativo y mejorando → buena señal LONG (momentum bajista agotándose)
        if mh_act < 0 and mh_act > mh_prev > mh_ant:
            score += 1.5; razones.append("MACD hist negativo 2 velas mejorando ✅ (suelo doble, momentum bajista agotado)")
        elif mh_act < 0 and mh_act > mh_prev:
            score += 1.0; razones.append("MACD hist negativo mejorando ✅ (suelo)")
        # MACD positivo y desacelerando → confirma subida ya en curso, señal débil para LONG predictivo
        elif mh_act > 0 and mh_act < mh_prev:
            score += 0.5; razones.append("MACD hist positivo desacelerando (confirma subida)")
        # MACD negativo y acelerando → fuerza bajista, EN CONTRA del LONG
        elif mh_act < 0 and mh_act < mh_prev < mh_ant:
            score -= 1.5; razones.append("⚠️ MACD hist negativo acelerando — fuerza bajista en contra del LONG")
        elif mh_act < 0 and mh_act < mh_prev:
            score -= 0.8; razones.append("⚠️ MACD hist negativo ganando fuerza bajista")
    # Cruce alcista inminente: MACD a punto de cruzar señal al alza desde negativo
    if len(macd_line_l) >= 2 and len(macd_sig_l) >= 2:
        ml_act = float(macd_line_l.iloc[-1]); ms_act = float(macd_sig_l.iloc[-1])
        ml_prev = float(macd_line_l.iloc[-2]); ms_prev = float(macd_sig_l.iloc[-2])
        dist_actual = ml_act - ms_act; dist_prev = ml_prev - ms_prev
        if dist_actual < 0 and dist_actual > dist_prev * 0.3 and dist_prev < 0:
            score += 1.2; razones.append("MACD cruce alcista inminente ✅ (líneas convergiendo desde negativo)")
    ...
    if ctx4h.get('disponible'):
        ...
        # ── MACD 4H: cruce bajista penaliza el LONG fuertemente ───────────
        if ctx4h.get('macd_cruce_bajista'):
            score -= 2.5
            razones.append("⚠️ ⚠️ MACD 4H cruce bajista CONFIRMADO — señal bajista fuerte en contra del LONG")
        elif ctx4h.get('macd_cruce_bajista_inm'):
            score -= 1.5
            razones.append("⚠️ MACD 4H cruce bajista inminente — momentum 4H girando bajista")
        elif ctx4h.get('macd_cruce_alcista'):
            score += 1.5
            razones.append("MACD 4H cruce alcista CONFIRMADO ✅ — señal alcista fuerte")
        elif ctx4h.get('macd_cruce_alcista_inm'):
            score += 0.8
            razones.append("MACD 4H cruce alcista inminente ✅")
        # Bonificar también cuando el histograma 4H lleva velas mejorando desde negativo
        mh4      = ctx4h.get('macd_hist', 0)
        mh4_prev = ctx4h.get('macd_hist_prev', 0)
        if mh4 < 0 and mh4 > mh4_prev:
            score += 0.8
            razones.append("MACD hist 4H negativo mejorando ✅ — momentum bajista agotándose")
...
def calcular_sl_tp_niveles(precio: float, atr: float, cfg: dict,
    ...
    if direccion == 'short':
        ...
        # Preferimos nivel con múltiples toques o EMA importante — no solo el más cercano
        sl_precio, sl_etiqueta = None, 'ATR_fallback'
        candidatos_sl = [(n, e, d) for n, e, d in resistencias
                         if sl_min_dist <= (n - precio) <= sl_max_dist]
        # Priorizar niveles estructurales fuertes (pico_local, MAX20, MAX50, EMA)
        ...
        # Si no hay estructural, tomar el primero disponible
        if sl_precio is None and candidatos_sl:
            nivel, etiqueta, _ = candidatos_sl[0]
            sl_precio = nivel + buffer; sl_etiqueta = etiqueta
        ...
        candidatos_sl = [(n, e, d) for n, e, d in soportes
                         if sl_min_dist <= (precio - n) <= sl_max_dist]
        for nivel, etiqueta, dist_pct in candidatos_sl:
