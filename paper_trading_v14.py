...
=============================================================
  ...
  ────────────────────────────────────────────────────────
  FIX CRÍTICO — MACD 4H (detectado en sesión 2026-04-29):
  ────────────────────────────────────────────────────────
  Problema: el bot abría SHORTs ignorando que el MACD en 4H
  estaba cruzando alcista (señal claramente en contra).
  El scoring solo evaluaba el MACD de 1H y usaba el contexto
  4H solo para tendencia por EMAs, sin ver el cruce MACD 4H.

  Fix aplicado en tres puntos:
  1. contexto_4h() ahora calcula y devuelve:
       · macd_hist, macd_hist_prev
       · macd_cruce_alcista  (cruce confirmado línea > señal)
       · macd_cruce_alcista_inm (inminente)
       · macd_cruce_bajista / macd_cruce_bajista_inm

  2. score_predictivo_short() penaliza con -2.5 si hay cruce
     alcista 4H confirmado, -1.5 si es inminente, y -1.0
     adicional si el histograma 4H lleva velas mejorando.
     Bonifica +1.5 si hay cruce bajista 4H confirmado.

  3. score_predictivo_long() aplica la lógica simétrica:
     penaliza cruces bajistas 4H, bonifica alcistas.

  Impacto: con este fix, BNB SHORT y SOL SHORT del 2026-04-29
  habrían sido bloqueados o su score habría bajado por debajo
  del mínimo al detectar el cruce alcista 4H.

  ────────────────────────────────────────────────────────
  FIX DE RENDIMIENTO — Diagnóstico pérdida $300 (~2 semanas):
  ────────────────────────────────────────────────────────
  Problema raíz: el bot abría demasiado fácil (score mínimo = 3,
  zona muerta muy permisiva) con apalancamiento ×12 (nivel2),
  y cerraba demasiado pronto por trailing antes de llegar al TP.
  Resultado: pérdidas pequeñas pero frecuentes sin ningún TP alcanzado.

  Cambios aplicados:

  1. TRAILING STOP menos agresivo
     · TRAILING_ATR_ACTIVACION: 1.5 → 2.5
       (se activa cuando el precio lleva más camino recorrido)
     · TRAILING_ATR_DISTANCIA: 1.2 → 2.0
       (deja más margen de retroceso antes de activar el cierre)

  2. Umbrales de nivel subidos — más difícil llegar a nivel2
     · SCORE_NIVEL2: 4.0 → 6.0
     · SCORE_NIVEL3: 6.0 → 8.0
     · SCORE_NIVEL4: 9.0 → 11.0
     (nivel2 = ×12 apalancamiento requiere señal más sólida)

  3. score_minimo() corregido — ya no devuelve siempre 3
     · Devuelve 5 por defecto (antes: 3 hardcodeado)
     · Esto obliga a señales de mayor calidad para abrir cualquier op.

  4. Zona muerta de penalización más estricta
     · FUERZA_MIN_PENALIZACION: 0.8 → 0.45
       (indicadores en contra empiezan a penalizar antes)
     · FACTOR_PENALIZACION: 0.5 → 0.65
       (cuando penalizan, lo hacen con más peso)

  5. Filtro IA más exigente
     · UMBRAL_IA_VETO: 0.60 → 0.55
       (la IA puede vetar con menos confianza)
     · SCORE_MINIMO_SIN_IA: 5.0 → 7.0
       (si la IA dice FLAT, el técnico necesita ser más sólido)

  6. Tiempo máximo reducido
     · MAX_HORAS_MAX: 120 → 72
       (limita la acumulación de funding en ops estancadas)
     · MAX_HORAS_BASE_SHORT: 16 → 14
     · MAX_HORAS_BASE_LONG: 20 → 18
=============================================================
"""
...
COMISION_PCT      = 0.001
SLIPPAGE_PCT      = 0.0005
FUNDING_RATE_HORA = 0.00005   # 0.04% cada 8h → 0.04% / 8 = 0.00005 por hora

MAX_MARGEN_TOTAL_PCT = 0.80

MAX_HORAS_BASE_LONG  = 18   # FIX: era 20
MAX_HORAS_BASE_SHORT = 14   # FIX: era 16
MAX_HORAS_ATR_REF    = 0.012
MAX_HORAS_MIN        = 6
MAX_HORAS_MAX        = 72   # FIX: era 120 — 5 días acumulaban demasiado funding

TRAILING_ATR_ACTIVACION = 2.5   # FIX: era 1.5 — se activaba demasiado pronto
TRAILING_ATR_DISTANCIA  = 2.0   # FIX: era 1.2 — dejaba muy poco margen de retroceso
...
UMBRAL_IA_VETO   = 0.55   # FIX: era 0.60 — veta con menos confianza necesaria

# Si score técnico es bueno pero IA dice FLAT:
# solo se bloquea si el score técnico es menor que este umbral.
SCORE_MINIMO_SIN_IA = 7.0  # FIX: era 5.0 — sin apoyo IA se necesita señal técnica más sólida
...
# Umbrales de score para selección de nivel de trade
SCORE_NIVEL4    = 11.0  # FIX: era 9.0  — nivel4 (x30 lev) requiere señal muy sólida
SCORE_NIVEL3    = 8.0   # FIX: era 6.0  — nivel3 (x20 lev)
SCORE_NIVEL2    = 6.0   # FIX: era 4.0  — nivel2 (x12 lev) era demasiado fácil de alcanzar
# (por debajo de SCORE_NIVEL2 → nivel1, x4 lev)

# Umbrales Bollinger Bands para niveles extremos en analizar_mercado
BB_EXTREMO_ALTO = 0.90  # BB position por encima → zona resistencia
BB_EXTREMO_BAJO = 0.10  # BB position por debajo → zona soporte

# Umbrales RSI 4H (contexto estructural — no adaptativos)
RSI4H_SOBRECOMPRA = 65
RSI4H_SOBREVENTA  = 35

# Umbrales Stochastic 1H
STOCH_SOBRECOMPRA      = 70   # zona sobrecompra
STOCH_SOBRECOMPRA_GIRO = 78   # sobrecompra con señal de giro (k < d)
STOCH_SOBREVENTA       = 30   # zona sobreventa
STOCH_SOBREVENTA_GIRO  = 22   # sobreventa con señal de giro (k > d)

# Distancia mínima al nivel para considerarlo "en zona" (% del precio)
DIST_NIVEL_ZONA_PCT = 0.5

# Umbrales de régimen de mercado
REGIMEN_ALCISTA_FUERTE =  2
REGIMEN_ALCISTA_DEBIL  =  1
REGIMEN_BAJISTA_FUERTE = -2
REGIMEN_BAJISTA_DEBIL  = -1

# ── Opción 1: Zona muerta de penalización ────────────────────────────────────
# FIX: FUERZA_MIN_PENALIZACION bajada de 0.8 a 0.45 — con 0.8 casi ningún indicador
# en contra penalizaba (RSI peso 0.8, stoch 0.7, vol 0.6 nunca llegaban al umbral).
# FIX: FACTOR_PENALIZACION subido de 0.5 a 0.65 — cuando penalizan, pesan más.
FUERZA_MIN_PENALIZACION = 0.45  # FIX: era 0.8 — ahora los indicadores en contra sí penalizan
FACTOR_PENALIZACION     = 0.65  # FIX: era 0.5 — penalización más significativa

# ── Opción 2: Pesos de fiabilidad por indicador ──────────────────────────────
# Multiplican la fuerza bruta del indicador — refleja su fiabilidad histórica.
# macd4h y tend4h son los más fiables (estructurales, timeframe superior).
# stoch y vol son los menos fiables (ruido, señales falsas frecuentes).
PESO_MACD4H   = 1.5   # cruce/histograma MACD en 4H — señal estructural
PESO_TEND4H   = 1.3   # tendencia EMA 4H — contexto dominante
PESO_MOM4H    = 1.1   # momentum 4H desacelerando — giro inminente
PESO_DIV      = 1.2   # divergencias — alta fiabilidad cuando son reales
PESO_MACD1H   = 1.0   # MACD 1H — base de referencia
PESO_NIVEL    = 1.0   # soporte/resistencia / fibonacci — base
PESO_RSI1H    = 0.8   # RSI 1H — muchos falsos positivos en rango
PESO_RSI4H    = 0.9   # RSI 4H — más fiable que 1H por timeframe
PESO_STOCH    = 0.7   # Stochastic — rápido, ruidoso
PESO_VOL      = 0.6   # Volumen — contexto, no señal directa
PESO_REG      = 0.5   # Régimen de mercado — lento, contextual

def score_minimo(prob: float, direccion: str) -> int:
    # FIX: antes devolvía siempre 3 (hardcodeado), lo que permitía entrar con
    # señales muy débiles. Ahora devuelve 5 como mínimo universal.
    # Con los nuevos umbrales de nivel (SCORE_NIVEL2=6), un score de 5 solo
    # abre en nivel1 (×4 lev), que es la posición más conservadora.
    return 5
...
def contexto_4h(simbolo: str) -> Dict:
    ...
    try:
        ...
        # ── Cruce MACD 4H (línea vs señal) ──────────────────────────────
        macd4_line = macd4_df.iloc[:, 0] if macd4_df.shape[1] > 0 else pd.Series([0])
        macd4_sig  = macd4_df.iloc[:, 1] if macd4_df.shape[1] > 1 else pd.Series([0])
        ml4_act  = float(macd4_line.iloc[-1]) if not macd4_line.empty else 0.0
        ms4_act  = float(macd4_sig.iloc[-1])  if not macd4_sig.empty  else 0.0
        ml4_prev = float(macd4_line.iloc[-2]) if len(macd4_line) > 1  else ml4_act
        ms4_prev = float(macd4_sig.iloc[-2])  if len(macd4_sig)  > 1  else ms4_act
        # Cruce alcista 4H: línea cruza señal al alza (dist negativa → positiva o convergiendo fuerte)
        dist4_act  = ml4_act  - ms4_act
        dist4_prev = ml4_prev - ms4_prev
        macd4_cruce_alcista = (dist4_act >= 0 and dist4_prev < 0)          # cruce confirmado
        macd4_cruce_alcista_inm = (dist4_act < 0 and dist4_act > dist4_prev * 0.3
                                   and dist4_prev < 0 and mh4 > mh4_prev)  # inminente
        macd4_cruce_bajista = (dist4_act <= 0 and dist4_prev > 0)
        macd4_cruce_bajista_inm = (dist4_act > 0 and dist4_act < dist4_prev * 0.3
                                   and dist4_prev > 0 and mh4 < mh4_prev)
        ...
        return {'disponible': True, 'tendencia': tendencia4, 'momentum': momentum4,
                ...
                'macd_hist': mh4, 'macd_hist_prev': mh4_prev,
                'macd_cruce_alcista': macd4_cruce_alcista,
                'macd_cruce_alcista_inm': macd4_cruce_alcista_inm,
                'macd_cruce_bajista': macd4_cruce_bajista,
                'macd_cruce_bajista_inm': macd4_cruce_bajista_inm}
    except Exception as e:
...
def evaluar_contexto_señal(df: pd.DataFrame, ultima: pd.Series,
    ...
    if conv_favor:
        mult_score *= 1.12; mult_margen *= 1.08
        # No se añade al log — ya aparece como señal a favor en el score directo
    if conv_contra:
    ...
    if nivel_favor['fuerza'] >= 2.0:
        ...
        # No se añade al log — ya aparece como patrón chartista en el score directo
...
def evaluar_macd_señal(df: pd.DataFrame, ctx4h: Dict) -> Dict:
    """
    Evalúa el MACD en 1H y 4H UNA SOLA VEZ y devuelve una señal unificada.
    Esta es la única fuente de verdad sobre el MACD para ambas funciones de scoring.
    Evita la doble interpretación donde el mismo indicador daba señal a favor
    tanto del LONG como del SHORT en la misma vela.

    Devuelve dict con:
      direccion_1h: 'alcista' | 'bajista' | 'neutral'
      fuerza_1h:    float (positivo=alcista, negativo=bajista)
      razon_1h:     texto para el log
      cruce_1h:     'alcista'|'alcista_inm'|'bajista'|'bajista_inm'|None
      hist_1h:      valor del histograma 1H
      direccion_4h: 'alcista' | 'bajista' | 'neutral'
      razon_4h:     texto para el log
    """
    result = {
        'direccion_1h': 'neutral', 'fuerza_1h': 0.0, 'razon_1h': '', 'cruce_1h': None,
        'hist_1h': 0.0, 'direccion_4h': 'neutral', 'razon_4h': '',
    }

    # ── MACD 1H ──────────────────────────────────────────────────────────
    mh = df['macd_hist'].dropna() if 'macd_hist' in df.columns else pd.Series(dtype=float)
    macd_line = df['macd'].dropna() if 'macd' in df.columns else pd.Series(dtype=float)
    macd_sig  = df['macd_signal'].dropna() if 'macd_signal' in df.columns else pd.Series(dtype=float)

    if len(mh) >= 3:
        mh_act  = float(mh.iloc[-1])
        mh_prev = float(mh.iloc[-2])
        mh_ant  = float(mh.iloc[-3])
        result['hist_1h'] = mh_act

        if mh_act < 0 and mh_act > mh_prev > mh_ant:
            result['direccion_1h'] = 'alcista'; result['fuerza_1h'] = 1.5
            result['razon_1h'] = "MACD hist negativo 2 velas mejorando ✅ (suelo doble, momentum bajista agotado)"
        elif mh_act < 0 and mh_act > mh_prev:
            result['direccion_1h'] = 'alcista'; result['fuerza_1h'] = 1.0
            result['razon_1h'] = "MACD hist negativo mejorando ✅ (suelo)"
        elif mh_act > 0 and mh_act > mh_prev > mh_ant:
            result['direccion_1h'] = 'alcista'; result['fuerza_1h'] = 1.0
            result['razon_1h'] = "MACD hist positivo acelerando — fuerza alcista"
        elif mh_act > 0 and mh_act > mh_prev:
            result['direccion_1h'] = 'alcista'; result['fuerza_1h'] = 0.5
            result['razon_1h'] = "MACD hist positivo ganando fuerza — alcista"
        elif mh_act > 0 and mh_act < mh_prev < mh_ant:
            result['direccion_1h'] = 'bajista'; result['fuerza_1h'] = -1.5
            result['razon_1h'] = "MACD hist positivo 2 velas decelerando ✅ (pico doble, momentum agotado)"
        elif mh_act > 0 and mh_act < mh_prev:
            result['direccion_1h'] = 'bajista'; result['fuerza_1h'] = -1.0
            result['razon_1h'] = "MACD hist positivo decelerando ✅ (pico)"
        elif mh_act < 0 and mh_act < mh_prev:
            result['direccion_1h'] = 'bajista'; result['fuerza_1h'] = -0.5
            result['razon_1h'] = "MACD hist negativo acelerando (bajista)"
        else:
            result['razon_1h'] = "MACD hist sin dirección clara"

    # Cruce inminente de líneas en 1H
    if len(macd_line) >= 2 and len(macd_sig) >= 2:
        ml_act  = float(macd_line.iloc[-1]); ms_act  = float(macd_sig.iloc[-1])
        ml_prev = float(macd_line.iloc[-2]); ms_prev = float(macd_sig.iloc[-2])
        dist_act  = ml_act  - ms_act
        dist_prev = ml_prev - ms_prev
        if dist_act >= 0 and dist_prev < 0:
            result['cruce_1h'] = 'alcista'
        elif dist_act < 0 and dist_prev < 0 and dist_act > dist_prev * 0.3:
            result['cruce_1h'] = 'alcista_inm'
        elif dist_act <= 0 and dist_prev > 0:
            result['cruce_1h'] = 'bajista'
        elif dist_act > 0 and dist_prev > 0 and dist_act < dist_prev * 0.3:
            result['cruce_1h'] = 'bajista_inm'

    # ── MACD 4H (ya calculado en ctx4h) ──────────────────────────────────
    if ctx4h.get('disponible'):
        if ctx4h.get('macd_cruce_alcista'):
            result['direccion_4h'] = 'alcista'
            result['razon_4h']     = "MACD 4H cruce alcista CONFIRMADO ✅"
        elif ctx4h.get('macd_cruce_alcista_inm'):
            result['direccion_4h'] = 'alcista'
            result['razon_4h']     = "MACD 4H cruce alcista inminente ✅"
        elif ctx4h.get('macd_cruce_bajista'):
            result['direccion_4h'] = 'bajista'
            result['razon_4h']     = "MACD 4H cruce bajista CONFIRMADO ✅"
        elif ctx4h.get('macd_cruce_bajista_inm'):
            result['direccion_4h'] = 'bajista'
            result['razon_4h']     = "MACD 4H cruce bajista inminente ✅"
        else:
            mh4      = ctx4h.get('macd_hist', 0)
            mh4_prev = ctx4h.get('macd_hist_prev', 0)
            if mh4 < 0 and mh4 > mh4_prev:
                result['direccion_4h'] = 'alcista'
                result['razon_4h']     = "MACD hist 4H negativo mejorando — momentum bajista agotándose"
            elif mh4 > 0 and mh4 < mh4_prev:
                result['direccion_4h'] = 'bajista'
                result['razon_4h']     = "MACD hist 4H positivo decelerando — momentum alcista agotándose"
            else:
                result['razon_4h'] = "MACD 4H sin dirección clara"

    return result


def analizar_mercado(df: pd.DataFrame, ultima: pd.Series, ctx4h: Dict,
                     divs: Dict = None, macd_señal: Dict = None) -> Dict:
    """
    Analiza el estado actual del mercado UNA SOLA VEZ y devuelve un dict
    neutral con la interpretación de cada indicador.

    Principio: cada indicador tiene UNA dirección (alcista / bajista / neutral)
    y una fuerza. Los scores de LONG y SHORT consumen este análisis de forma
    simétrica: lo que suma al LONG resta al SHORT y viceversa.

    Esto elimina el problema de doble interpretación donde el mismo indicador
    aparecía como señal a favor de LONG *y* de SHORT en la misma vela.
    """
    a = {}   # análisis — dict de retorno
    ...
    # ══ RSI 1H ══════════════════════════════════════════════════════════════
    # Calculamos umbrales adaptativos una sola vez para ambas direcciones.
    umbral_sc, umbral_sv = calcular_rsi_umbral(rsi_serie, 'short')  # sobrecompra / sobreventa
    if rsi >= umbral_sc:
        a['rsi_dir'] = 'bajista'; a['rsi_f'] = 1.5
        a['rsi_r']   = f"RSI={rsi:.0f} sobrecomprado (umbral={umbral_sc:.0f})"
    elif rsi >= umbral_sc - 8:
        a['rsi_dir'] = 'bajista'; a['rsi_f'] = 0.5
        a['rsi_r']   = f"RSI={rsi:.0f} acercándose a sobrecompra"
    elif rsi <= umbral_sv:
        a['rsi_dir'] = 'alcista'; a['rsi_f'] = 1.5
        a['rsi_r']   = f"RSI={rsi:.0f} sobrevendido (umbral={umbral_sv:.0f})"
    elif rsi <= umbral_sv + 8:
        a['rsi_dir'] = 'alcista'; a['rsi_f'] = 0.5
        a['rsi_r']   = f"RSI={rsi:.0f} acercándose a sobreventa"
    else:
        a['rsi_dir'] = 'neutral'; a['rsi_f'] = 0.0; a['rsi_r'] = ''

    # ══ RSI 4H ══════════════════════════════════════════════════════════════
    r4 = ctx4h.get('rsi', 50) if ctx4h.get('disponible') else 50
    if r4 >= RSI4H_SOBRECOMPRA:
        a['rsi4_dir'] = 'bajista'; a['rsi4_f'] = 0.5
        a['rsi4_r']   = f"RSI 4H={r4:.0f} sobrecomprado (umbral={RSI4H_SOBRECOMPRA})"
    elif r4 <= RSI4H_SOBREVENTA:
        a['rsi4_dir'] = 'alcista'; a['rsi4_f'] = 0.5
        a['rsi4_r']   = f"RSI 4H={r4:.0f} sobrevendido (umbral={RSI4H_SOBREVENTA})"
    else:
        a['rsi4_dir'] = 'neutral'; a['rsi4_f'] = 0.0; a['rsi4_r'] = ''

    # ══ Stochastic 1H ═══════════════════════════════════════════════════════
    stk = float(ultima.get('stoch_k', 50)); std_val = float(ultima.get('stoch_d', 50))
    if stk >= STOCH_SOBRECOMPRA_GIRO and stk < std_val + 2:
        a['stoch_dir'] = 'bajista'; a['stoch_f'] = 1.0
        a['stoch_r']   = f"Stoch={stk:.0f} sobrecomprado y girando"
    elif stk >= STOCH_SOBRECOMPRA:
        a['stoch_dir'] = 'bajista'; a['stoch_f'] = 0.5; a['stoch_r'] = ''
    elif stk <= STOCH_SOBREVENTA_GIRO and stk > std_val - 2:
        a['stoch_dir'] = 'alcista'; a['stoch_f'] = 1.0
        a['stoch_r']   = f"Stoch={stk:.0f} sobrevendido y girando"
    elif stk <= STOCH_SOBREVENTA:
        a['stoch_dir'] = 'alcista'; a['stoch_f'] = 0.5; a['stoch_r'] = ''
    else:
        a['stoch_dir'] = 'neutral'; a['stoch_f'] = 0.0; a['stoch_r'] = ''

    # ══ MACD 1H ═════════════════════════════════════════════════════════════
    # Ya calculado por evaluar_macd_señal() — pasamos los campos relevantes
    if macd_señal:
        a['macd1h_dir']   = macd_señal.get('direccion_1h', 'neutral')
        a['macd1h_f']     = abs(macd_señal.get('fuerza_1h', 0.0))
        a['macd1h_r']     = macd_señal.get('razon_1h', '')
        a['macd1h_cruce'] = macd_señal.get('cruce_1h')   # 'alcista'|'bajista'|'*_inm'|None
    else:
        a['macd1h_dir'] = 'neutral'; a['macd1h_f'] = 0.0
        a['macd1h_r'] = ''; a['macd1h_cruce'] = None

    # ══ MACD 4H ═════════════════════════════════════════════════════════════
    if macd_señal:
        a['macd4h_dir'] = macd_señal.get('direccion_4h', 'neutral')
        a['macd4h_r']   = macd_señal.get('razon_4h', '')
    else:
        a['macd4h_dir'] = 'neutral'; a['macd4h_r'] = ''

    # ══ Tendencia 4H (EMA) ══════════════════════════════════════════════════
    if ctx4h.get('disponible'):
        t4 = ctx4h['tendencia']; m4 = ctx4h['momentum']
        if t4 == 'alcista':
            a['tend4h_dir'] = 'alcista'; a['tend4h_f'] = 2.0
            a['tend4h_r']   = "4H ALCISTA"
        elif t4 == 'bajista':
            a['tend4h_dir'] = 'bajista'; a['tend4h_f'] = 2.0
            a['tend4h_r']   = "4H BAJISTA"
        elif t4 == 'lateral' and 'alcista' in m4:
            a['tend4h_dir'] = 'alcista'; a['tend4h_f'] = 1.0
            a['tend4h_r']   = "4H lateral momentum alcista"
        elif t4 == 'lateral' and 'bajista' in m4:
            a['tend4h_dir'] = 'bajista'; a['tend4h_f'] = 1.0
            a['tend4h_r']   = "4H lateral momentum bajista"
        else:
            a['tend4h_dir'] = 'neutral'; a['tend4h_f'] = 0.0; a['tend4h_r'] = ''

        # Momentum 4H desacelerando (señal de giro inminente)
        if m4 == 'bajista_desacelerando':
            a['mom4h_giro'] = 'alcista'; a['mom4h_f'] = 1.0
            a['mom4h_r']    = "4H bajista desacelerando"
        elif m4 == 'alcista_desacelerando':
            a['mom4h_giro'] = 'bajista'; a['mom4h_f'] = 1.0
            a['mom4h_r']    = "4H alcista desacelerando"
        else:
            a['mom4h_giro'] = 'neutral'; a['mom4h_f'] = 0.0; a['mom4h_r'] = ''
    else:
        for k in ('tend4h_dir','tend4h_r','mom4h_giro','mom4h_r'):
            a[k] = 'neutral' if 'dir' in k or 'giro' in k else ''
        a['tend4h_f'] = 0.0; a['mom4h_f'] = 0.0

    # ══ Fibonacci / distancia a soporte / resistencia ═══════════════════════
    ...
    if dist_res < DIST_NIVEL_ZONA_PCT:
        a['nivel_dir'] = 'bajista'; a['nivel_f'] = 1.0
        a['nivel_r']   = f"Precio en resistencia ({dist_res:.2f}%)"
    elif cerca_618 == 1:
        a['nivel_dir'] = 'bajista'; a['nivel_f'] = 1.0
        a['nivel_r']   = "Fibonacci 0.618 (resistencia)"
    elif bb_pos > BB_EXTREMO_ALTO:
        a['nivel_dir'] = 'bajista'; a['nivel_f'] = 0.5
        a['nivel_r']   = f"BB superior ({bb_pos:.2f})"
    elif dist_sop < DIST_NIVEL_ZONA_PCT:
        a['nivel_dir'] = 'alcista'; a['nivel_f'] = 1.0
        a['nivel_r']   = f"Precio en soporte ({dist_sop:.2f}%)"
    elif cerca_382 == 1 or cerca_500 == 1:
        a['nivel_dir'] = 'alcista'; a['nivel_f'] = 1.0
        a['nivel_r']   = "Fibonacci 0.382/0.5 (soporte)"
    elif bb_pos < BB_EXTREMO_BAJO:
        a['nivel_dir'] = 'alcista'; a['nivel_f'] = 0.5
        a['nivel_r']   = f"BB inferior ({bb_pos:.2f})"
    else:
        a['nivel_dir'] = 'neutral'; a['nivel_f'] = 0.0; a['nivel_r'] = ''

    # ══ Divergencias ════════════════════════════════════════════════════════
    # Las divergencias ya tienen dirección propia — solo hay que mapearlas
    a['div_dir'] = 'neutral'; a['div_f'] = 0.0; a['div_razones'] = []
    if divs:
        if divs.get('div_bajista_rsi'):
            f = divs.get('div_bajista_rsi_fuerza', 0.5)
            a['div_dir'] = 'bajista'; a['div_f'] += 1.0 + f * 0.5
            a['div_razones'].append(f"Divergencia bajista RSI real (fuerza={f:.2f})")
        if divs.get('div_bajista_macd'):
            f = divs.get('div_bajista_macd_fuerza', 0.5)
            if a['div_dir'] != 'alcista':
                a['div_dir'] = 'bajista'; a['div_f'] += 1.0 + f * 0.5
            a['div_razones'].append(f"Divergencia bajista MACD real (fuerza={f:.2f})")
        if divs.get('div_alcista_rsi'):
            f = divs.get('div_alcista_rsi_fuerza', 0.5)
            a['div_dir'] = 'alcista'; a['div_f'] += 1.0 + f * 0.5
            a['div_razones'].append(f"Divergencia alcista RSI real (fuerza={f:.2f})")
        if divs.get('div_alcista_macd'):
            f = divs.get('div_alcista_macd_fuerza', 0.5)
            if a['div_dir'] != 'bajista':
                a['div_dir'] = 'alcista'; a['div_f'] += 1.0 + f * 0.5
            a['div_razones'].append(f"Divergencia alcista MACD real (fuerza={f:.2f})")
        if divs.get('convergencia_bajista'):
            if a['div_dir'] != 'alcista':
                a['div_dir'] = 'bajista'; a['div_f'] += 0.8
            a['div_razones'].append("MACD hist convergiendo bajista")
        if divs.get('convergencia_alcista'):
            if a['div_dir'] != 'bajista':
                a['div_dir'] = 'alcista'; a['div_f'] += 0.8
            a['div_razones'].append("MACD convergiendo alcista")
        # Si hay divergencias en ambas direcciones → señal contradictoria → anulan
        bajista_presente = any('bajista' in r for r in a['div_razones'])
        alcista_presente = any('alcista' in r for r in a['div_razones'])
        if bajista_presente and alcista_presente:
            a['div_dir'] = 'neutral'; a['div_f'] = 0.0
            a['div_razones'] = ["⚠️ Divergencias contradictorias — anulan"]
    else:
        # Fallback a columnas básicas si no hay objeto divs
        if int(ultima.get('divergencia_bajista_rsi', 0)) == 1:
            a['div_dir'] = 'bajista'; a['div_f'] = 0.8
            a['div_razones'].append("Div bajista RSI (señal básica)")
        elif int(ultima.get('divergencia_alcista_rsi', 0)) == 1:
            a['div_dir'] = 'alcista'; a['div_f'] = 0.8
            a['div_razones'].append("Div alcista RSI (señal básica)")

    # ══ Volumen ══════════════════════════════════════════════════════════════
    vr = df['volumen_ratio'].dropna()
    c_series = df['close']
    precio_sube = precio > float(c_series.iloc[-3]) if len(c_series) >= 3 else False
    precio_baja = precio < float(c_series.iloc[-3]) if len(c_series) >= 3 else False
    a['vol_dir'] = 'neutral'; a['vol_f'] = 0.0; a['vol_r'] = ''
    if len(vr) >= 3:
        vol_bajo = float(vr.iloc[-1]) < 0.80 and float(vr.iloc[-2]) < 0.80
        if precio_sube and vol_bajo:
            a['vol_dir'] = 'bajista'; a['vol_f'] = 1.0
            a['vol_r']   = "Volumen bajo en subida (fuerza alcista débil)"
        elif precio_baja and vol_bajo:
            a['vol_dir'] = 'alcista'; a['vol_f'] = 1.0
            a['vol_r']   = "Volumen bajo en bajada (vendedores agotados)"

    # ══ Régimen de mercado ═══════════════════════════════════════════════════
    reg = int(ultima.get('regimen_mercado', 0)) if not pd.isna(ultima.get('regimen_mercado', np.nan)) else 0
    if reg >= REGIMEN_ALCISTA_FUERTE:
        a['reg_dir'] = 'alcista'; a['reg_f'] = 0.5; a['reg_r'] = f"Régimen alcista ({reg})"
    elif reg >= REGIMEN_ALCISTA_DEBIL:
        a['reg_dir'] = 'alcista'; a['reg_f'] = 0.3; a['reg_r'] = ''
    elif reg <= REGIMEN_BAJISTA_FUERTE:
        a['reg_dir'] = 'bajista'; a['reg_f'] = 0.5; a['reg_r'] = f"Régimen bajista ({reg})"
    elif reg <= REGIMEN_BAJISTA_DEBIL:
        a['reg_dir'] = 'bajista'; a['reg_f'] = 0.3; a['reg_r'] = ''
    else:
        a['reg_dir'] = 'neutral'; a['reg_f'] = 0.0; a['reg_r'] = ''

    return a


def _aplicar_indicador(score: float, razones: list,
                       direccion_mercado: str, fuerza_bruta: float, razon: str,
                       dir_trade: str, peso: float = 1.0) -> float:
    """
    Aplica un indicador al score con pesos de fiabilidad (Opción 2) y
    zona muerta de penalización (Opción 1).

    Opción 2 — Pesos:
      La fuerza bruta del indicador se multiplica por su peso de fiabilidad
      antes de aplicarse. MACD 4H y tendencia 4H pesan más que stoch o vol.

    Opción 1 — Zona muerta:
      Si el indicador apunta en CONTRA del trade pero su fuerza (ya pesada)
      es menor que FUERZA_MIN_PENALIZACION, NO penaliza — simplemente no
      confirma. Solo penaliza al 50% (FACTOR_PENALIZACION) si supera el umbral.
      Esto evita que indicadores ruidosos en zona media bloqueen señales válidas.
    """
    if direccion_mercado == 'neutral' or fuerza_bruta == 0:
        return score

    fuerza = fuerza_bruta * peso   # fuerza ajustada por fiabilidad

    if direccion_mercado == dir_trade:
        score += fuerza
        if razon: razones.append(f"✅ {razon}")
    else:
        # Zona muerta: señales débiles en contra no penalizan
        if fuerza < FUERZA_MIN_PENALIZACION:
            if razon: razones.append(f"〰️ {razon} — señal débil, no penaliza")
        else:
            penalizacion = fuerza * FACTOR_PENALIZACION
            score -= penalizacion
            if razon: razones.append(f"⚠️ {razon} — en contra del {dir_trade.upper()} (-{penalizacion:.1f})")
    return score


def score_predictivo_short(df: pd.DataFrame, ultima: pd.Series,
                            ctx4h: Dict, patrones: Dict = None,
                            divs: Dict = None, prob_ct: float = None,
                            macd_señal: Dict = None) -> Tuple[float, List[str]]:
    """
    Score para SHORT. Consume el análisis de mercado unificado con:
    - Opción 1: zona muerta — indicadores débiles en contra no penalizan
    - Opción 2: pesos de fiabilidad por indicador
    """
    score = 0.0; razones = []
    a = analizar_mercado(df, ultima, ctx4h, divs=divs, macd_señal=macd_señal)
    def ap(d, f, r, peso=1.0):
        nonlocal score
        score = _aplicar_indicador(score, razones, d, f, r, 'bajista', peso)

    ap(a['rsi_dir'],    a['rsi_f'],    a['rsi_r'],    PESO_RSI1H)
    ap(a['rsi4_dir'],   a['rsi4_f'],   a['rsi4_r'],   PESO_RSI4H)
    ap(a['stoch_dir'],  a['stoch_f'],  a['stoch_r'],  PESO_STOCH)
    ap(a['macd1h_dir'], a['macd1h_f'], a['macd1h_r'], PESO_MACD1H)

    # Cruce MACD 1H — señal puntual, aplica peso directamente
    cruce = a.get('macd1h_cruce')
    if cruce == 'bajista':
        v = round(1.2 * PESO_MACD1H, 2); score += v
        razones.append(f"✅ MACD cruce bajista confirmado 1H (+{v})")
    elif cruce == 'bajista_inm':
        v = round(0.8 * PESO_MACD1H, 2); score += v
        razones.append(f"✅ MACD cruce bajista inminente 1H (+{v})")
    elif cruce == 'alcista':
        # Cruce alcista confirmado es señal fuerte → supera zona muerta siempre
        v = round(1.2 * PESO_MACD1H * FACTOR_PENALIZACION, 2); score -= v
        razones.append(f"⚠️ MACD cruce alcista confirmado 1H — en contra del SHORT (-{v})")
    elif cruce == 'alcista_inm':
        raw = 0.8 * PESO_MACD1H
        if raw >= FUERZA_MIN_PENALIZACION:
            v = round(raw * FACTOR_PENALIZACION, 2); score -= v
            razones.append(f"⚠️ MACD cruce alcista inminente 1H — en contra del SHORT (-{v})")
        else:
            razones.append("〰️ MACD cruce alcista inminente 1H — señal débil, no penaliza")

    ap(a['macd4h_dir'],  1.5 if a['macd4h_dir'] == 'bajista' else 2.0, a['macd4h_r'],   PESO_MACD4H)
    ap(a['tend4h_dir'],  a['tend4h_f'],  a['tend4h_r'],  PESO_TEND4H)
    ap(a['mom4h_giro'],  a['mom4h_f'],   a['mom4h_r'],   PESO_MOM4H)
    ap(a['nivel_dir'],   a['nivel_f'],   a['nivel_r'],   PESO_NIVEL)
    ap(a['div_dir'],     a['div_f'],     ', '.join(a['div_razones']), PESO_DIV)
    ap(a['vol_dir'],     a['vol_f'],     a['vol_r'],     PESO_VOL)
    ap(a['reg_dir'],     a['reg_f'],     a['reg_r'],     PESO_REG)
...
def score_predictivo_long(df: pd.DataFrame, ultima: pd.Series,
                           ...
                           macd_señal: Dict = None) -> Tuple[float, List[str]]:
    """
    Score para LONG. Espejo exacto del SHORT con:
    - Opción 1: zona muerta — indicadores débiles en contra no penalizan
    - Opción 2: pesos de fiabilidad por indicador
    """
    score = 0.0; razones = []
    ...
    def ap(d, f, r, peso=1.0):
        nonlocal score
        score = _aplicar_indicador(score, razones, d, f, r, 'alcista', peso)
    ...
    if cruce == 'alcista':
        v = round(1.2 * PESO_MACD1H, 2); score += v
        razones.append(f"✅ MACD cruce alcista confirmado 1H (+{v})")
    elif cruce == 'alcista_inm':
        v = round(0.8 * PESO_MACD1H, 2); score += v
        razones.append(f"✅ MACD cruce alcista inminente 1H (+{v})")
    elif cruce == 'bajista':
        v = round(1.2 * PESO_MACD1H * FACTOR_PENALIZACION, 2); score -= v
        razones.append(f"⚠️ MACD cruce bajista confirmado 1H — en contra del LONG (-{v})")
    elif cruce == 'bajista_inm':
        raw = 0.8 * PESO_MACD1H
        if raw >= FUERZA_MIN_PENALIZACION:
            v = round(raw * FACTOR_PENALIZACION, 2); score -= v
            razones.append(f"⚠️ MACD cruce bajista inminente 1H — en contra del LONG (-{v})")
        else:
            razones.append("〰️ MACD cruce bajista inminente 1H — señal débil, no penaliza")

    ap(a['macd4h_dir'],  1.5 if a['macd4h_dir'] == 'alcista' else 2.0, a['macd4h_r'],   PESO_MACD4H)
    ap(a['tend4h_dir'],  a['tend4h_f'],  a['tend4h_r'],  PESO_TEND4H)
    ap(a['mom4h_giro'],  a['mom4h_f'],   a['mom4h_r'],   PESO_MOM4H)
    ap(a['nivel_dir'],   a['nivel_f'],   a['nivel_r'],   PESO_NIVEL)
    ap(a['div_dir'],     a['div_f'],     ', '.join(a['div_razones']), PESO_DIV)
...
def calcular_sl_tp_niveles(precio: float, atr: float, cfg: dict,
    ...
    if direccion == 'short':
        # ── SL: buscar resistencia estructural por encima del precio ──
        # REGLA CRÍTICA: el SL de un SHORT debe ir SIEMPRE por encima de la
        # resistencia que motiva la entrada. Si la resistencia señal está en X,
        # el SL va en X + buffer — nunca entre el precio y esa resistencia.
        sl_precio, sl_etiqueta = None, 'ATR_fallback'

        # Todos los candidatos por encima del precio, sin filtro sl_min_dist inicial
        # (lo aplicamos después de añadir el buffer sobre la resistencia)
        candidatos_sl = [(n, e, d) for n, e, d in resistencias
                         if (n - precio) > 0 and (n - precio) <= sl_max_dist]
        candidatos_sl.sort(key=lambda x: x[2])  # más cercana primero

        for nivel, etiqueta, dist_pct in candidatos_sl:
            # SL siempre POR ENCIMA de la resistencia — buffer amplio
            sl_candidato = nivel + buffer * 2
            sl_dist_candidato = sl_candidato - precio
            # Si la distancia resultante excede el máximo, descartar
            if sl_dist_candidato > sl_max_dist:
                continue
            # Preferir niveles estructurales, pero aceptar cualquiera si es el único
            if etiqueta in ('pico_local', 'MAX20', 'MAX50', 'EMA50', 'EMA200', 'SMA50', 'SMA200'):
                sl_precio = sl_candidato; sl_etiqueta = etiqueta; break
            elif sl_precio is None:
                sl_precio = sl_candidato; sl_etiqueta = etiqueta

        # Fallback ATR si no hay ningún nivel
        if sl_precio is None:
            sl_precio = precio + atr * min(cfg['sl_mult'], SL_ATR_MAX_MULT)
            sl_etiqueta = 'ATR_fallback'
            print(f"    ⚠️  SL SHORT sin nivel estructural — usando ATR×{cfg['sl_mult']:.1f}")
        sl_dist = sl_precio - precio

        # ── TP: buscar soporte alcanzable con RR suficiente ──
        tp_precio, tp_etiqueta = None, 'ATR_fallback'
        # En swing podemos apuntar al segundo soporte
        ...
        if tp_precio is None:
            # Fallback: tp_ratio × sl_dist, pero nunca más de tp_max_dist
            tp_precio = precio - min(sl_dist * max(cfg['tp_ratio'], rr_min + 0.1), tp_max_dist)
            tp_etiqueta = 'ATR_fallback'
            print(f"    ⚠️  TP SHORT sin soporte alcanzable — usando ratio×{cfg['tp_ratio']:.1f}")

    else:  # long
        # ── SL: buscar soporte estructural por debajo del precio ──
        # REGLA CRÍTICA: el SL de un LONG debe ir SIEMPRE por debajo del soporte
        # que motiva la entrada. Si el soporte está en X, el SL va en X - buffer,
        # nunca entre ese soporte y el precio.
        sl_precio, sl_etiqueta = None, 'ATR_fallback'

        # Todos los candidatos por debajo del precio, sin filtro sl_min_dist inicial
        candidatos_sl = [(n, e, d) for n, e, d in soportes
                         if (precio - n) > 0 and (precio - n) <= sl_max_dist]
        candidatos_sl.sort(key=lambda x: x[2])  # más cercano primero

        for nivel, etiqueta, dist_pct in candidatos_sl:
            # SL siempre POR DEBAJO del soporte — buffer amplio
            sl_candidato = nivel - buffer * 2
            sl_dist_candidato = precio - sl_candidato
            if sl_dist_candidato > sl_max_dist:
                continue  # demasiado lejos
            if etiqueta in ('valle_local', 'MIN20', 'MIN50', 'EMA50', 'EMA200', 'SMA50', 'SMA200'):
                sl_precio = sl_candidato; sl_etiqueta = etiqueta; break
            elif sl_precio is None:
                sl_precio = sl_candidato; sl_etiqueta = etiqueta
        ...
        if sl_precio is None:
            sl_precio = precio - atr * min(cfg['sl_mult'], SL_ATR_MAX_MULT)
            sl_etiqueta = 'ATR_fallback'
            print(f"    ⚠️  SL LONG sin nivel estructural — usando ATR×{cfg['sl_mult']:.1f}")
        sl_dist = precio - sl_precio

        # ── TP: buscar resistencia alcanzable con RR suficiente ──
        tp_precio, tp_etiqueta = None, 'ATR_fallback'
        ...
        if tp_precio is None:
            tp_precio = precio + min(sl_dist * max(cfg['tp_ratio'], rr_min + 0.1), tp_max_dist)
            tp_etiqueta = 'ATR_fallback'
            print(f"    ⚠️  TP LONG sin resistencia alcanzable — usando ratio×{cfg['tp_ratio']:.1f}")
    ...
    # ── Validación extra: RR mínimo y TP no demasiado lejano ──
    if rr_real < rr_min:
        return {'viable': False, 'motivo': f'R:R={rr_real:.2f} < mínimo {rr_min}'}

    print(f"    📍 SL={sl_etiqueta} ({sl_dist_final/precio*100:.2f}% del precio) | "
          f"TP={tp_etiqueta} ({tp_dist_final/precio*100:.2f}%) | R:R={rr_real:.2f}")

    return {'viable': True, 'sl': round(sl_precio, 8), 'tp': round(tp_precio, 8),
...
def calcular_max_horas(atr_pct: float, direccion: str,
                       tp_dist_pct: float = None) -> int:
    """
    Tiempo máximo dinámico basado en ATR y distancia al TP.
    A mayor TP, más tiempo para alcanzarlo.

    Ejemplos con BTC (ATR ~0.54%):
      TP 1%  → ~37h  |  TP 2% → ~74h  |  TP 3% → ~111h  |  TP 4% → 120h (tope)
    """
    base = MAX_HORAS_BASE_LONG if direccion == 'long' else MAX_HORAS_BASE_SHORT
    factor_atr = (MAX_HORAS_ATR_REF / atr_pct) if atr_pct > 0 else 1.0
    factor_tp  = max(1.0, tp_dist_pct / 1.0) if tp_dist_pct and tp_dist_pct > 0 else 1.0
    horas = base * factor_atr * factor_tp
    return int(np.clip(horas, MAX_HORAS_MIN, 120))
...
def gestionar_posiciones(estado: Dict, precio: float, atr: float,
    ...
    for pos in estado['posiciones']:
        ...
        tp_dist_pos = abs(pos['tp'] - pos['precio_entrada']) / pos['precio_entrada'] * 100 if 'tp' in pos else None
        max_h = pos.get('max_horas', calcular_max_horas(atr_pct_pos, pos['dir'], tp_dist_pct=tp_dist_pos))
        ...
        if pos['dir'] == 'long':
            ...
            if not cerrar:
                sl_tocado = precio <= pos['sl'] or (low is not None and low <= pos['sl'])
                tp_tocado = precio >= pos['tp'] or (high is not None and high >= pos['tp'])
                if sl_tocado:   cerrar = True; motivo = 'trailing_stop' if pos.get('trailing') else 'stop_loss'
                elif tp_tocado: cerrar = True; motivo = 'take_profit'
                elif horas >= max_h: cerrar = True; motivo = 'tiempo_maximo'
            if cerrar:
                # SL/TP siempre liquidan al precio del nivel fijado (no al close de vela)
                precio_salida = pos['tp'] if motivo == 'take_profit' else (pos['sl'] if motivo in ('stop_loss', 'trailing_stop') else precio)
                ps = precio_salida * (1 - SLIPPAGE_PCT); exp = pos['margen'] * pos['lev']
                ret = (ps - pos['precio_entrada']) / pos['precio_entrada']
                # Funding: solo periodos de 8h completos (como en los mercados reales)
                periodos_funding = int(horas // 8)
                fund = exp * FUNDING_RATE_HORA * 8 * periodos_funding
                ...
                telegram(f"{emoji} <b>CIERRE LONG {simbolo}</b> [{VERSION}]\n"
                         ...
                         f"P&L: ${pnl:+.2f} | Funding: ${fund:.2f} ({periodos_funding}×8h)\n"
            ...
            if not cerrar:
                sl_tocado = precio >= pos['sl'] or (high is not None and high >= pos['sl'])
                tp_tocado = precio <= pos['tp'] or (low is not None and low <= pos['tp'])
                if sl_tocado:   cerrar = True; motivo = 'trailing_stop' if pos.get('trailing') else 'stop_loss'
                elif tp_tocado: cerrar = True; motivo = 'take_profit'
                elif horas >= max_h: cerrar = True; motivo = 'tiempo_maximo'
            if cerrar:
...
def abrir_posicion(estado: Dict, simbolo: str, precio: float, atr: float,
    ...
    if score_adj >= SCORE_NIVEL4:    nivel_str, cfg = 'nivel4', TRADE_CONFIG['nivel4']
    elif score_adj >= SCORE_NIVEL3:  nivel_str, cfg = 'nivel3', TRADE_CONFIG['nivel3']
    elif score_adj >= SCORE_NIVEL2:  nivel_str, cfg = 'nivel2', TRADE_CONFIG['nivel2']
    else:                   nivel_str, cfg = 'nivel1', TRADE_CONFIG['nivel1']

    # ── Filtro de tendencia 4H por nivel ──────────────────────────────────
    # Lógica simétrica para LONG y SHORT:
    #
    # SHORT:
    #   · 4H bajista  → mantiene todas las posiciones (4→4, 3→3, 2→2, 1→1)
    #   · 4H lateral  → baja un nivel (4→3, 3→2, mantiene 2, mantiene 1)
    #   · 4H alcista  → baja dos niveles (4→2, 3→1, no abre 2, no abre 1)
    #
    # LONG:
    #   · 4H alcista  → mantiene todas las posiciones (4→4, 3→3, 2→2, 1→1)
    #   · 4H lateral  → baja un nivel (4→3, 3→2, mantiene 2, mantiene 1)
    #   · 4H bajista  → baja dos niveles (4→2, 3→1, no abre 2, no abre 1)
    t4h = (ctx4h or {}).get('tendencia', 'lateral')

    tendencia_a_favor  = ('bajista' if dir_ == 'short' else 'alcista')
    tendencia_en_contra = ('alcista' if dir_ == 'short' else 'bajista')

    if t4h == tendencia_en_contra:
        # 4H en contra: bajar dos niveles (4→2, 3→1, bloquear 2 y 1)
        if nivel_str == 'nivel4':
            nivel_str, cfg = 'nivel2', TRADE_CONFIG['nivel2']
            print(f"  ⬇️  4H {t4h.upper()} en contra — nivel bajado de 4 a 2")
        elif nivel_str == 'nivel3':
            nivel_str, cfg = 'nivel1', TRADE_CONFIG['nivel1']
            print(f"  ⬇️  4H {t4h.upper()} en contra — nivel bajado de 3 a 1")
        elif nivel_str in ('nivel2', 'nivel1'):
            print(f"  ❌ {nivel_str.upper()} {dir_.upper()} bloqueado — 4H {t4h.upper()} en contra (mínimo nivel3 para operar)")
            return
    elif t4h == 'lateral':
        # 4H lateral: bajar un nivel (4→3, 3→2, mantener 2 y 1)
        if nivel_str == 'nivel4':
            nivel_str, cfg = 'nivel3', TRADE_CONFIG['nivel3']
            print(f"  ⬇️  4H LATERAL — nivel bajado de 4 a 3")
        elif nivel_str == 'nivel3':
            nivel_str, cfg = 'nivel2', TRADE_CONFIG['nivel2']
            print(f"  ⬇️  4H LATERAL — nivel bajado de 3 a 2")
        # nivel2 y nivel1 se mantienen sin cambio en lateral
    # else: 4H a favor → sin cambio de nivel

    print(f"  📊 Nivel por score ({score_adj:.1f}): {nivel_str.upper()} "
          f"(x{cfg['leverage']} lev, {cfg['margen']*100:.0f}% margen) | 4H={t4h.upper()}")
    ...
    max_horas = calcular_max_horas(atr_pct, dir_, tp_dist_pct=ctx_sltp.get('tp_dist_pct'))
...
def ejecutar():
    ...
    for simbolo in CRIPTOS:
        ...
        # Extender high/low con la vela en curso para capturar SL/TP tocados intravela
        high = max(high, float(vela_actual['high']))
        low  = min(low,  float(vela_actual['low']))
        ...
        # ── Señal MACD unificada — calculada UNA SOLA VEZ para ambos scores ──
        macd_s = evaluar_macd_señal(df_pred, ctx4h)

        if n_longs == 0:
            score_l, razones_l = score_predictivo_long(df_pred, ultima, ctx4h, patrones, divs, macd_señal=macd_s)
        ...
        if n_shorts == 0:
            score_s, razones_s = score_predictivo_short(df_pred, ultima, ctx4h, patrones, divs, macd_señal=macd_s)
