...
COMISION_PCT      = 0.001
SLIPPAGE_PCT      = 0.0005
FUNDING_RATE_HORA = 0.00000125  # 0.01% cada 8h → 0.00125% por hora (tasa real de exchanges)
    ...
    for pos in estado['posiciones']:
        ...
        if pos['dir'] == 'long':
            ...
            if cerrar:
                ...
                # Calculamos margen y posiciones SIN contar la que acabamos de cerrar
                margen_restante = sum(p['margen'] for p in estado['posiciones'] if p is not pos)
                pos_restantes   = len(estado['posiciones']) - 1
                telegram(f"{emoji} <b>CIERRE LONG {simbolo}</b> [{VERSION}]\n"
                         ...
                         f"🔒 Margen restante en uso: ${margen_restante:.2f} | Posiciones: {pos_restantes}")
            else:
                nuevas.append(pos)
        else:  # short
            if precio < pos.get('precio_ref', pos['precio_entrada']):
