...
# ── IA DESACTIVADA: la función ya no depende de prob ni de umbrales del modelo ──
# El score mínimo ahora es fijo (3), equivalente al caso más conservador del original.
# Cuando el modelo esté calibrado, restaurar la lógica original descomentando lo de abajo.
def score_minimo(prob: float, direccion: str) -> int:
    # ── LÓGICA ORIGINAL (IA activa) — COMENTADA ──────────────────────────────
    ...
    # ── FIN LÓGICA ORIGINAL ──────────────────────────────────────────────────
    return 3  # fijo: siempre se exigen 3 condiciones técnicas (más conservador)
...
def predecir_tendencia_diaria(df1d: pd.DataFrame, sub: Dict) -> float:
    ...
    try:
        ...
        if len(df_d) < 10:
            return 0.50
        prob = predecir(df_d, sub)
        # Asegurar que el resultado es un float válido
...
def amplificar_nivel_por_ia(nivel_str: str, prob_1d: float, direccion: str,
    ...
    """
    ── IA DESACTIVADA ──
    Esta función originalmente modificaba el nivel de apalancamiento usando la
    probabilidad del modelo sobre datos diarios (tendencia macro).
    Actualmente devuelve siempre el nivel sin cambio.

    Cuando el modelo esté calibrado, restaurar la lógica original descomentando
    el bloque de abajo y eliminando el return al final.
    """
    # ── LÓGICA ORIGINAL (IA activa) — COMENTADA ──────────────────────────────
    ...
    # IA desactivada: nivel sin cambio siempre
    nota = f"🚫 IA diaria DESACTIVADA — nivel sin cambio ({nivel_str})"
    return nivel_str, nota
...
def score_predictivo_short(df: pd.DataFrame, ultima: pd.Series,
    ...
    # TODO: desactivado temporalmente — el modelo necesita revisión antes de
    # usarlo como indicador en el score. Las probabilidades actuales no son
    # fiables para este uso (prob_ct ≈ prob_long porque usan los mismos datos).
    # Cuando el modelo esté calibrado, descomentar el bloque de abajo.
    #
...
def abrir_posicion(estado: Dict, simbolo: str, precio: float, atr: float,
    ...
    # ── Asignar nivel directamente desde el score predictivo ajustado ──
    # El score_adj ya integra: indicadores técnicos + contexto 4H + divergencias
    # + patrones chartistas + multiplicador de contexto.
    # Umbrales calibrados para que el mercado lateral normal caiga en nivel1-2
    # y las señales de alta confluencia lleguen a nivel3-4.
    #
    # score_adj < 4      → nivel1 (x4,  margen 8%)  — señal mínima, bajo riesgo
    # score_adj 4–6      → nivel2 (x12, margen 14%) — señal moderada
    # score_adj 6–9      → nivel3 (x20, margen 18%) — señal fuerte
    # score_adj >= 9     → nivel4 (x30, margen 22%) — señal de muy alta confluencia
    #
    # ── Sistema original calcular_calidad_score — CONSERVADO COMENTADO ──────
    # (dependía de ADX del 4H que Kraken no devuelve → siempre 0 → nivel siempre 1)
    # ultima_cal = ultima_row.copy() if ultima_row is not None else pd.Series()
    # if df_completo is not None and 'macd_hist' in df_completo.columns and len(df_completo) >= 3:
    #     mh_series = df_completo['macd_hist'].dropna()
    #     if len(mh_series) >= 2:
    #         ultima_cal['macd_hist_prev'] = float(mh_series.iloc[-2])
    # pts_calidad, notas_calidad = calcular_calidad_score(ultima_cal, ctx4h or {}, divs or {}, dir_)
    # nivel_str, cfg = asignar_nivel(pts_calidad)
    # ── FIN SISTEMA ORIGINAL ──────────────────────────────────────────────────

    if score_adj >= 9.0:
        nivel_str, cfg = 'nivel4', TRADE_CONFIG['nivel4']
    elif score_adj >= 6.0:
    ...
    print(f"  📊 Calidad por score ({score_adj:.1f}): {nivel_str.upper()} "
          f"(x{cfg['leverage']} lev, {cfg['margen']*100:.0f}% margen)"
          f"  [<4→niv1 | 4–6→niv2 | 6–9→niv3 | 9+→niv4]")

    # ── Amplificador IA diario (tendencia macro 1D) — DESACTIVADO ──
    # El bloque de abajo llamaba a predecir_tendencia_diaria() + amplificar_nivel_por_ia()
    # para modificar el nivel ANTES de calcular SL/TP y margen.
    # Se conserva el código comentado para reactivarlo cuando el modelo esté calibrado.
    ...
    # ── FIN LÓGICA ORIGINAL ──────────────────────────────────────────────────
    print(f"  🌐 IA diaria: DESACTIVADA — nivel sin cambio ({nivel_str})")
    ...
    telegram(f"🚀 <b>APERTURA {dir_.upper()} {simbolo}</b> [{VERSION}]{swing_tag}\n"
             f"Modo: {modo} | Nivel: {nivel_str.upper()} (score={score_adj:.1f}) | Prob: {prob:.3f}\n"
             f"Razones:\n{razones_str}\n"
...
def ejecutar():
    ...
    for simbolo in CRIPTOS:
        ...
        # ── Bloques 5 y 6: calcular scores de LONG y SHORT antes de decidir ─
        # Primero se calculan ambos scores. Luego se aplica el filtro de
        # contradicción: si ambos superan el mínimo y no hay diferencia clara
        # de dirección, el mercado es ambiguo y no se abre ninguno.
        # Reglas:
        #   · Diferencia mínima entre scores: SCORE_DIFF_MINIMA (2.0 pts)
        #   · Si solo uno supera el mínimo → se abre ese
        #   · Si los dos superan el mínimo pero la diferencia es < 2.0 → ambiguo, no abre
        #   · Si los dos superan el mínimo y uno gana por >= 2.0 → se abre el ganador
        #
        # GIRO DE POSICIÓN:
        #   Si hay una posición abierta y la señal contraria es nivel3+ (score>=6),
        #   y la posición actual está en beneficio neto (después de comisiones y funding),
        #   se cierra la posición actual y se abre la contraria.
        #   El cierre se registra como 'giro' (neutro, no cuenta en WR).
        SCORE_DIFF_MINIMA  = 2.0
        SCORE_MINIMO_GIRO  = 6.0   # nivel3+ para permitir el giro

        # ── Función auxiliar: calcular PnL neto de una posición al precio actual ──
        def pnl_neto_posicion(pos, precio_actual):
            """Retorna el PnL neto en $ considerando comisiones + funding acumulado."""
            exp   = pos['margen'] * pos['lev']
            horas = max((hora_unix - pos.get('ts_apertura', hora_unix)) / 3600, 0)
            fund  = exp * FUNDING_RATE_HORA * horas
            if pos['dir'] == 'long':
                ps  = precio_actual * (1 - SLIPPAGE_PCT)
                ret = (ps - pos['precio_entrada']) / pos['precio_entrada']
            else:
                ps  = precio_actual * (1 + SLIPPAGE_PCT)
                ret = (pos['precio_entrada'] - ps) / pos['precio_entrada']
        ...
        # ── Función auxiliar: cerrar posición por giro ───────────────────────────
        def cerrar_posicion_por_giro(pos, precio_actual):
            """Cierra una posición con motivo 'giro'. No cuenta en WR."""
            exp   = pos['margen'] * pos['lev']
            horas = max((hora_unix - pos.get('ts_apertura', hora_unix)) / 3600, 0)
            fund  = exp * FUNDING_RATE_HORA * horas
            if pos['dir'] == 'long':
            ...
            estado['pnl_total']     += pnl
            estado['funding_total'] += fund
            # No incrementa n_ops ni n_wins/n_loses → neutro en WR
            ...
            telegram(f"🔄 <b>GIRO {pos['dir'].upper()} → {'SHORT' if pos['dir']=='long' else 'LONG'} {nombre}</b> [{VERSION}]\n"
                     ...
                     f"P&L: ${pnl:+.2f} | Funding: ${fund:.2f}\n"
                     f"💰 Capital: ${estado['capital']:,.2f} ({'+' if rent_now>=0 else ''}{rent_now:.2f}%)")
            print(f"  🔄 GIRO: cerrado {pos['dir'].upper()} con P&L=${pnl:+.2f} (neutro en WR)")
            return pnl

        # ── Pre-cálculo LONG ─────────────────────────────────────────
        score_l_adj  = None   # None = no calculado (ya hay posición abierta o sin modelo)
        score_s_adj  = None
        ctx_señal_l  = None
        ctx_señal_s  = None
        razones_l    = []
        ...
        if n_longs == 0:
            ...
            if sub_long:
                # ── IA DESACTIVADA — predicciones del modelo comentadas ──────────
        ...
        min_s = score_minimo(1.0, 'long')  # = 3, simétrico
        ...
        if l_valido and s_valido:
            ...
            if diff < SCORE_DIFF_MINIMA:
                ...
                # Solo se permite el de mayor score
        ...
        # ── Bloque 5: mostrar y ejecutar LONG ────────────────────────
        print(f"\n  {'─'*25} ANÁLISIS LONG {'─'*20}")
        resumen_l = "—"
        if n_longs > 0:
            ...
            print(f"  IA DESACTIVADA (prob fija=1.0)  score_base={score_l:.1f}  "
                  f"mult_ctx=×{ctx_señal_l['multiplicador_score']:.2f}  confianza=BLOQUEADO")
            print(f"  🚫 LONG BLOQUEADO — {ctx_señal_l['razon_bloqueo']}")
            resumen_l = "BLOQUEADO (soporte/resistencia fuerte)"
        elif score_l_adj is None:
            resumen_l = "—"
        else:
            # ── Caso especial: hay SHORT abierto y la señal LONG es nivel3+ ──
            ...
            if pos_short_abierto is not None:
                # Hay un short abierto — solo actuar si la señal LONG es nivel3+ (score>=6)
                print(f"  IA DESACTIVADA (prob fija=1.0)  score_base={score_l:.1f}  "
                ...
                for r in razones_favor: print(f"    ✅ {r}") if razones_favor else print("    (ninguna)")
                print(f"  Score ajustado: {score_l:.1f} × {ctx_señal_l['multiplicador_score']:.2f}"
                      f" = {score_l_adj:.1f}  (mínimo giro={SCORE_MINIMO_GIRO})")
                if score_l_adj >= SCORE_MINIMO_GIRO:
                    pnl_actual = pnl_neto_posicion(pos_short_abierto, precio)
                    if pnl_actual > 0:
                        print(f"  🔄 GIRO posible — SHORT en beneficio neto (${pnl_actual:+.2f}) + LONG score={score_l_adj:.1f} ≥ {SCORE_MINIMO_GIRO}")
                        cerrar_posicion_por_giro(pos_short_abierto, precio)
                        n_shorts = 0  # actualizar contador local
                        print(f"  🟢 SEÑAL LONG (GIRO) — ABRIENDO POSICIÓN")
                        abrir_posicion(estado, simbolo, precio_entrada, atr,
                                       ...
                                       score_adj=score_l_adj)
                        resumen_l = f"✅ LONG abierto (giro desde SHORT)"
                    else:
                        print(f"  ❌ Giro denegado — SHORT en pérdida neta (${pnl_actual:+.2f}), no se gira")
                        resumen_l = f"no gira — SHORT en pérdida (${pnl_actual:+.2f})"
                else:
                    print(f"  ❌ Señal insuficiente para giro (score={score_l_adj:.1f} < {SCORE_MINIMO_GIRO} nivel3+)")
                    resumen_l = f"no gira — score {score_l_adj:.1f} < {SCORE_MINIMO_GIRO}"
            else:
                # Sin posición contraria — flujo normal
                print(f"  IA DESACTIVADA (prob fija=1.0)  score_base={score_l:.1f}  "
                      f"mult_ctx=×{ctx_señal_l['multiplicador_score']:.2f}  "
                      f"marg=×{ctx_señal_l['multiplicador_margen']:.2f}  "
                      f"confianza={ctx_señal_l['nivel_confianza'].upper()}")
        ...
        # ── Bloque 6: mostrar y ejecutar SHORT ───────────────────────
        ...
        if n_shorts > 0:
            ...
            print(f"  IA DESACTIVADA (prob fija=1.0)  score_base={score_s:.1f}  "
                  f"mult_ctx=×{ctx_señal_s['multiplicador_score']:.2f}  confianza=BLOQUEADO")
            print(f"  🚫 SHORT BLOQUEADO — {ctx_señal_s['razon_bloqueo']}")
            resumen_s = "BLOQUEADO (soporte/resistencia fuerte)"
        elif score_s_adj is None:
            resumen_s = "—"
        else:
            # ── Caso especial: hay LONG abierto y la señal SHORT es nivel3+ ──
            pos_long_abierto = next(
                (p for p in estado['posiciones'] if p['simbolo'] == nombre and p['dir'] == 'long'), None)
            if pos_long_abierto is not None:
                # Hay un long abierto — solo actuar si la señal SHORT es nivel3+ (score>=6)
                print(f"  IA DESACTIVADA (prob fija=1.0)  score_base={score_s:.1f}  "
                ...
                for r in razones_favor_s: print(f"    ✅ {r}") if razones_favor_s else print("    (ninguna)")
                print(f"  Score ajustado: {score_s:.1f} × {ctx_señal_s['multiplicador_score']:.2f}"
                      f" = {score_s_adj:.1f}  (mínimo giro={SCORE_MINIMO_GIRO})")
                if score_s_adj >= SCORE_MINIMO_GIRO:
                    pnl_actual = pnl_neto_posicion(pos_long_abierto, precio)
                    if pnl_actual > 0:
                        print(f"  🔄 GIRO posible — LONG en beneficio neto (${pnl_actual:+.2f}) + SHORT score={score_s_adj:.1f} ≥ {SCORE_MINIMO_GIRO}")
                        cerrar_posicion_por_giro(pos_long_abierto, precio)
                        n_longs = 0  # actualizar contador local
                        print(f"  🔴 SEÑAL SHORT (GIRO) — ABRIENDO POSICIÓN")
                        abrir_posicion(estado, simbolo, precio_entrada, atr,
                                       prob_short, {}, 'short', 'short', ts, hora_unix,
                                       razones_s + ctx_señal_s['contexto_log'],
                                       df_pred, ctx4h, ultima, divs,
                                       ctx1d=ctx1d, sub_modelo=sub_short,
                                       score_adj=score_s_adj)
                        resumen_s = f"✅ SHORT abierto (giro desde LONG)"
                    else:
                        print(f"  ❌ Giro denegado — LONG en pérdida neta (${pnl_actual:+.2f}), no se gira")
                        resumen_s = f"no gira — LONG en pérdida (${pnl_actual:+.2f})"
                else:
                    print(f"  ❌ Señal insuficiente para giro (score={score_s_adj:.1f} < {SCORE_MINIMO_GIRO} nivel3+)")
                    resumen_s = f"no gira — score {score_s_adj:.1f} < {SCORE_MINIMO_GIRO}"
            else:
                # Sin posición contraria — flujo normal
                print(f"  IA DESACTIVADA (prob fija=1.0)  score_base={score_s:.1f}  "
                      f"mult_ctx=×{ctx_señal_s['multiplicador_score']:.2f}  "
