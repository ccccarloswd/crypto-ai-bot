...
# sl_max y tp_max se calculan en tiempo real desde el ATR (ver calcular_sl_tp_niveles).
LONG_CONFIG = {
    'elite':   {'leverage': 24, 'margen': 0.22, 'sl_mult': 1.4, 'tp_ratio': 3.0},
    'premium': {'leverage': 15, 'margen': 0.20, 'sl_mult': 1.7, 'tp_ratio': 2.6},
}
SHORT_CONFIG = {
    'elite':   {'leverage': 18, 'margen': 0.18, 'sl_mult': 1.5, 'tp_ratio': 3.0},
    'premium': {'leverage': 12, 'margen': 0.15, 'sl_mult': 1.7, 'tp_ratio': 2.6},
}

# ── Parámetros de Risk/Reward ──
RR_MINIMO          = 2.0    # ratio mínimo TP/SL para abrir operación
SL_ATR_MIN_MULT    = 0.5    # SL nunca menor a 0.5×ATR (evita wicks de ruido)
SL_ATR_MAX_MULT    = 4.0    # SL nunca mayor a 4×ATR (limita riesgo máximo)
TP_ATR_MAX_MULT    = 10.0   # TP nunca mayor a 10×ATR
BUFFER_NIVEL_PCT   = 0.003  # 0.3% extra más allá del nivel para el SL
...
def score_predictivo_short(df: pd.DataFrame, ultima: pd.Series,
                            ...
                            divs: Dict = None) -> Tuple[float, List[str]]:
    """
...
def calcular_sl_tp_niveles(precio: float, atr: float, cfg: dict,
    ...
    Lógica para SHORT:
      · SL: justo POR ENCIMA de la resistencia más cercana + buffer (0.3%)
             Si no hay resistencia útil en rango, SL = precio + SL_ATR_MAX_MULT × ATR
      · TP: justo POR ENCIMA del soporte más cercano (para salir antes de que rebote)
             Si no hay soporte útil, TP = precio − cfg.tp_ratio × ATR

    Lógica para LONG (simétrica):
      · SL: justo POR DEBAJO del soporte más cercano − buffer
      · TP: justo POR DEBAJO de la resistencia más cercana

    Límites dinámicos basados en ATR (no porcentajes fijos):
      · SL mínimo: SL_ATR_MIN_MULT × ATR (no entrar si el ruido ya lo toca)
      · SL máximo: SL_ATR_MAX_MULT × ATR (no arriesgar más de N ATRs)
      · TP máximo: TP_ATR_MAX_MULT × ATR

    Filtro R:R: si TP/SL < RR_MINIMO → rechazar operación (devuelve viable=False).
    """
    resistencias = niveles['resistencias']
    soportes     = niveles['soportes']

    sl_min_dist = atr * SL_ATR_MIN_MULT   # distancia mínima absoluta del SL
    sl_max_dist = atr * SL_ATR_MAX_MULT   # distancia máxima absoluta del SL
    tp_max_dist = atr * TP_ATR_MAX_MULT   # distancia máxima absoluta del TP

    buffer = precio * BUFFER_NIVEL_PCT    # 0.3% del precio como margen de seguridad

    # Contexto 4H para expandir el TP en swing
    es_swing = (
    ...
    if direccion == 'short':
        ...
        for nivel, etiqueta, dist_pct in resistencias:
            ...
            if dist_abs < sl_min_dist:
                continue  # demasiado cerca: el ruido ya lo tocaría
            if dist_abs > sl_max_dist:
                break     # demasiado lejos: riesgo excesivo
            sl_precio   = nivel + buffer  # SL un poco POR ENCIMA de la resistencia
            sl_etiqueta = etiqueta
            break

        if sl_precio is None:
            # Sin resistencia útil: SL a 2×ATR (fallback conservador)
            sl_precio   = precio + atr * min(cfg['sl_mult'], SL_ATR_MAX_MULT)
            sl_etiqueta = 'ATR_fallback'

        sl_dist = sl_precio - precio  # distancia absoluta (positivo = SL por encima)

        # ── TP: por encima del soporte más cercano (salir antes del rebote) ──
        tp_precio = None
        tp_etiqueta = 'ATR_fallback'

        # En swing con tendencia 4H bajista podemos buscar el segundo soporte
        n_soporte_objetivo = 1 if not es_swing else min(2, len(soportes))

        for i, (nivel, etiqueta, dist_pct) in enumerate(soportes):
            if i < n_soporte_objetivo - 1:
                continue  # saltamos el primer soporte en swing para ir más lejos
            tp_candidato = nivel + buffer  # TP un poco POR ENCIMA del soporte
            tp_dist = precio - tp_candidato
            if tp_dist < sl_min_dist:
                continue  # soporte demasiado cerca
            if tp_dist > tp_max_dist:
                break
            rr = tp_dist / sl_dist
            if rr < RR_MINIMO:
                continue  # R:R insuficiente con este TP, buscar soporte más lejano
            tp_precio   = tp_candidato
            tp_etiqueta = etiqueta
            break

        if tp_precio is None:
            # Fallback: TP según ratio objetivo
            tp_dist_obj = sl_dist * max(cfg['tp_ratio'], RR_MINIMO + 0.1)
            tp_precio   = precio - min(tp_dist_obj, tp_max_dist)
            tp_etiqueta = 'ATR_fallback'
        ...
        for nivel, etiqueta, dist_pct in soportes:
            ...
            if dist_abs > sl_max_dist:
                break
            sl_precio   = nivel - buffer  # SL un poco POR DEBAJO del soporte
            sl_etiqueta = etiqueta
            break
        ...
        sl_dist = precio - sl_precio  # distancia absoluta (positivo)

        # ── TP: por debajo de la resistencia más cercana ─────────────────
        tp_precio = None
        tp_etiqueta = 'ATR_fallback'
        n_res_objetivo = 1 if not es_swing else min(2, len(resistencias))

        for i, (nivel, etiqueta, dist_pct) in enumerate(resistencias):
            if i < n_res_objetivo - 1:
                continue
            tp_candidato = nivel - buffer  # TP un poco POR DEBAJO de la resistencia
    ...
    # ── Verificación final de R:R ─────────────────────────────────────────
    if direccion == 'short':
        sl_dist_final = sl_precio - precio
        tp_dist_final = precio - tp_precio
    else:
        sl_dist_final = precio - sl_precio
        tp_dist_final = tp_precio - precio

    # Aseguramos que SL y TP están al lado correcto del precio
    ...
    return {
        ...
        'motivo': '' if viable else f'R:R={rr_real:.2f} < mínimo {RR_MINIMO}',
    }
...
def abrir_posicion(estado: Dict, simbolo: str, precio: float, atr: float,
    ...
    # ── ATR relativo para ajustes dinámicos ──
    atr_pct = atr / precio if precio > 0 else MAX_HORAS_ATR_REF

    # ── Calcular SL/TP desde niveles reales de mercado ──
    ctx_sltp = None
    if df_completo is not None:
        ultima_row = df_completo.iloc[-1]
        niveles    = _niveles_mercado(df_completo, ultima_row, precio, atr)
        ctx_sltp   = calcular_sl_tp_niveles(precio, atr, cfg, dir_, niveles, ctx4h or {})

    if ctx_sltp is None or not ctx_sltp['viable']:
        motivo_rechazo = ctx_sltp['motivo'] if ctx_sltp else 'sin contexto de mercado'
        print(f"  ❌ {nombre} {dir_.upper()} rechazado: {motivo_rechazo}")
        return

    sl  = ctx_sltp['sl']
    tp  = ctx_sltp['tp']
    liq = calcular_liquidacion(precio, cfg, dir_, 1.0)  # margen aún no calculado, usamos ratio puro

    # Aplicar slippage al precio de entrada
    pe = precio * (1 + SLIPPAGE_PCT) if dir_ == 'long' else precio * (1 - SLIPPAGE_PCT)

    # ── Margen ajustado por volatilidad ──
    ...
    if not limite_margen_ok(estado, margen_r):
        ...
        print(f"  ⚠️  Margen total agotado: ${margen_usado:.2f} / ${estado['capital']:.2f}, skip")
        return

    # ── Tiempo máximo dinámico ──
    ...
    telegram(f"🚀 <b>APERTURA {dir_.upper()} {simbolo}</b> [{VERSION}]{swing_tag}\n"
             f"Modo: {modo} | Calidad: {calidad.upper()} | Prob: {prob:.3f}\n"
             f"Razones:\n{razones_str}\n"
...
def ejecutar():
    ...
    for simbolo in CRIPTOS:
        ...
        # Detección de patrones chartistas (1H + confirmación 4H)
        df4h_raw = ctx4h.get('df') if ctx4h.get('disponible') else None
        patrones = detectar_patrones(df, df4h_raw)
        if patrones['patrones_short'] or patrones['patrones_long']:
            print(f"  📐 Patrones SHORT: {patrones['patrones_short']} (score={patrones['score_short']})")
            print(f"  📐 Patrones LONG:  {patrones['patrones_long']} (score={patrones['score_long']})")
        if patrones['niveles_clave']:
            niveles_str = ' | '.join(f"{k}=${v:,.2f}" for k, v in patrones['niveles_clave'][:3])
            print(f"  🎯 Niveles clave: {niveles_str}")
        ...
        # Precio de entrada: usamos el close de la vela actual (en formación),
        # que refleja el precio real de mercado en el momento de ejecutar la señal.
        # La vela [-2] (cerrada) se usa solo para gestionar posiciones ya abiertas.
        ...
        # ── Divergencias reales (compartidas entre long y short) ──
        divs = detectar_divergencias(df_pred, ventana=40)
        if divs['resumen']:
            print(f"  🔀 Divergencias: {' | '.join(divs['resumen'])}")
        ...
        if n_longs == 0:
            ...
            if sub_long:
                ...
                # Contexto global de señal (fuerza de niveles, divergencias, 4H)
                ...
                if ctx_señal_l['bloquear']:
                    ...
                    confianza   = ctx_señal_l['nivel_confianza'].upper()
                    mult_m      = ctx_señal_l['multiplicador_margen']
                    print(f"  LONG  prob={prob_long:.4f} score={score_l:.1f}→{score_l_adj:.1f}/{min_s_l} "
                          f"confianza={confianza} marg×{mult_m:.2f} | ${precio:,.2f}")
                    if ctx_señal_l['contexto_log']:
                        print(f"  Contexto: {' | '.join(ctx_señal_l['contexto_log'])}")

                    if prob_long >= PROB_PREMIUM_LONG and score_l_adj >= min_s_l:
                        res = clasificar_long(prob_long, score_l_adj)
                        if res:
                            cal, cfg = res
                            # Pasar multiplicador de margen al abrir posición
                            cfg_mod = dict(cfg)
                            cfg_mod['margen'] = cfg['margen'] * mult_m
                            abrir_posicion(estado, simbolo, precio_entrada, atr,
                                           prob_long, cfg_mod, cal, 'long', ts, hora_unix,
            ...
            if max_g_long < 0.03:
                ...
                if sub_short:
                    ...
                    if ctx_señal_s['bloquear']:
                        ...
                        confianza   = ctx_señal_s['nivel_confianza'].upper()
                        mult_m      = ctx_señal_s['multiplicador_margen']
                        print(f"  SHORT prob={prob_short:.4f} score={score_s:.1f}→{score_s_adj:.1f}/{min_s_s} "
                              f"confianza={confianza} marg×{mult_m:.2f}")
                        if ctx_señal_s['contexto_log']:
                            print(f"  Contexto: {' | '.join(ctx_señal_s['contexto_log'])}")

                        if prob_short >= PROB_PREMIUM_SHORT and score_s_adj >= min_s_s:
                            res = clasificar_short(prob_short, score_s_adj)
                            if res:
                                cal, cfg = res
                                cfg_mod = dict(cfg)
                                cfg_mod['margen'] = cfg['margen'] * mult_m
                                abrir_posicion(estado, simbolo, precio_entrada, atr,
                                               prob_short, cfg_mod, cal, 'short', ts, hora_unix,
